...
> Belongs to: [Reference](命令.md)　·　Related: [Config file (sectioned version)](配置文件.md)

This page shows the **complete default raw text** of `plugins/QinhItems/config.yml`, with line-by-line annotations after each section. For the topic-grouped explanatory version, see [config.yml full config](配置文件.md).
...
For the mechanism see [Layers and Assembly → Write domain policy](../04-开发者/层与装配.md#3-写域策略writedomainpolicy); for related errors see [Validation error quick reference §9](校验报错速查.md#9-写域违规writedomainpolicy).
...
- `action-routing.strict: true` — Logic fields (if/when/switch…) appearing in an action raise an error; `false` only warns. See [Action system → Design](../02-服主指南/动作系统/概览.md#3-关键设计yaml-里不能写逻辑).
...
For full details see [Gem sockets](../02-服主指南/宝石孔.md) and [Integrations](../04-开发者/集成.md).
...
See [Attributes and values](../02-服主指南/属性与数值.md).
...
See [Attributes and values](../02-服主指南/属性与数值.md#3-配置属性映射).

---

## Next steps

- [config.yml full config (topic-grouped version)](配置文件.md)
