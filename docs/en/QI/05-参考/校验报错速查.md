...
> Section: [Reference](./commands.md)　·　Related: [Item Definition](../02-server-guide/item-definition.md) · [Resource Pack](../02-server-guide/resource-pack.md) · [Layers & Assembly](../04-developer/layers-assembly.md)
...
| `unknown type=$type` | Type is not in `item_types.yml` | Use a valid type, see [Item Types](../02-server-guide/item-types.md) |
...
All are **errors**. See [Resource Pack → Validation Constraints](../02-server-guide/resource-pack.md#2-校验约束).
...
> Warnings do not block saving, but at runtime that handler will return `HANDLER_UNAVAILABLE`. For the handler list, see [Handlers](../02-server-guide/action-system/handlers.md).
...
**Allowed payload keys**: `value` / `raw` / `data` / `payload`. See [Providers & Bridges](../04-developer/providers-bridges.md).
...
For complex logic, [develop a handler](../04-developer/handler-development.md).
...
`strict: true` (default) is an error, `false` is a warning. Three domains: INSTANCE / LAYER / RUNTIME, see [Layers & Assembly → Write-Domain Policy](../04-developer/layers-assembly.md#3-写域策略writedomainpolicy).
...
- [Item Definition](../02-server-guide/item-definition.md): the correct syntax
- [Resource Pack](../02-server-guide/resource-pack.md): model isolation rules
- [Commands](./commands.md): `/qi diagnose` / `/qi problems`
