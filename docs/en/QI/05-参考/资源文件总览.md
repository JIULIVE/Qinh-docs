...
> Belongs to: [Reference](命令.md)　·　Related: [Config File](配置文件.md) · [Built-in Content Inventory](内置内容清单.md)
...
> These YAML files **all carry Chinese comments at the top**. This page does not repeat the original text; it only gives "which file manages what + key structure + jump links". The only page with line-by-line annotations is the [config.yml original text page](配置文件原文.md).
...
|---|---|---|
| `config.yml` | Main config (directories / write domains / action routing / gem bridge / combat / binding / attribute mapping) | [Config File](配置文件.md) · [Original Text](配置文件原文.md) |
| `item_types.yml` | Item types and capabilities (see full table below) | [Item Types](../02-服主指南/物品类型.md) |
| `item_tiers.yml` | Tiers (weight / capacity / glow) | [Tiers & Display](../02-服主指南/品质与显示.md) · [Built-in Inventory](内置内容清单.md#一内置品质item_tiersyml) |
| `enchant_limits.yml` | Total enchant level caps (by type / tier) | [Enchant Limits](../02-服主指南/附魔上限.md) |
| `plugin.yml` | Plugin metadata / permissions / dependencies | [Permissions](权限.md) · [Config File §plugin.yml](配置文件.md#pluginyml-元数据) |

## 2. Content Directories

| Directory / File | Manages | Details |
|---|---|---|
| `items/<type>.yml` | Item definitions (one type per file, ~140 examples) | [Item Definitions](../02-服主指南/物品定义.md) · [Example Library](../02-服主指南/物品示例库.md) |
| `fragments/*.yml` | Fragments (reusable snippets) | [Fragments & Templates](../02-服主指南/碎片与模板.md) |
| `sections/*.yml` | Sections / affix pools / tier pools + affixes sections | [Sections](../02-服主指南/段.md) · [Affixes](../02-服主指南/词缀.md) |
| `sets/*.yml` | Set definitions | [Sets](../02-服主指南/套装.md) · [Built-in Inventory](内置内容清单.md#四内置套装setssetsyml) |

## 3. Integration Directory integrations/

| File | Manages | Details |
|---|---|---|
| `gem_socket_types.yml` | Gem socket types (mapping LI / MG backends) | [Gem Sockets](../02-服主指南/宝石孔.md) |
| `legendinlay_sockets.yml` | Legendinlay socket catalog (lore must match LI verbatim) | [Gem Sockets](../02-服主指南/宝石孔.md#4-孔-lore-渲染优先级) |
| `magicgem_sockets.yml` | MagicGem socket catalog | Same as above |
| `legendinlay_guide.yml` / `magicgem_guide.yml` | Backend integration notes (documentation-type, self-commented) | [Integration Practice](../04-开发者/集成实操.md) |
| `action_handlers.yml` | Action handler namespace notes (documentation-type) | [Handlers](../02-服主指南/动作系统/处理器.md) |
| `bridges.yml` | Bridge switches | [Integration](../04-开发者/集成.md) |
| `resource_models.yml` | Editor model catalog (models listed by category) | [Resource Pack](../02-服主指南/资源包.md#3-模型目录编辑器用) |
...
| `capabilities` | Capability flags, see [Item Types](../02-服主指南/物品类型.md#1-能力typecapability是什么) |
...
The fields `display` / `legendinlay` / `magicgem` / `lore` are covered in [Gem Sockets](../02-服主指南/宝石孔.md#2-定义孔类型).

---

## Next Steps

- [Config File Original Text](配置文件原文.md): config.yml line by line
- [Built-in Content Inventory](内置内容清单.md): tiers / sections / sets
- [Item Types](../02-服主指南/物品类型.md): capability matrix
