...
> Part of: [Reference](./commands.md)　·　Related: [Validation Error Quick Reference](./validation-errors.md) · [Commands](./commands.md)
...
For error meanings, see [Validation Error Quick Reference](./validation-errors.md).
...
For details, see [Attributes & Values](../02-server-guide/attributes-numbers.md#6-没装-attributeplus-会怎样).
...
For details, see [Integration Practice → Legendinlay](../04-developer/integration-howto.md#三legendinlay宝石孔后端-a).

---

## 5. Interpreting Health Metrics

An item is only considered healthy when its `healthIssues()` is empty. Unhealthy items still load but are listed as problems. For common causes of unhealthiness, see [Validation Error Quick Reference §1](./validation-errors.md#1-物品健康检查qinhitemdefinitionhealthissues).
...
Refresh = `rebuild()` + `variables().refresh()` on each QI item, preserving instance / seed. See [Random Generation → Refresh Service](../02-server-guide/random-generation.md#7-物品刷新服务).

---

## Next Steps

- [Validation Error Quick Reference](./validation-errors.md): Each error + how to fix
- [Commands](./commands.md): Diagnostic commands
- [FAQ](./faq.md)
