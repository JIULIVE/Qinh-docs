...
> Section: [Developer](./api-overview.md)　·　Related: [Integration](./integration.md) · [Integration in Practice](./integration-howto.md) · [Attributes & Numbers](../02-server-guide/attributes-numbers.md)
...
Diagnostics: `ItemManagerAPI.diagnose(ref)` returns a `DiagnosticResult`, with codes including `SOURCE_NOT_FOUND` (prefix not registered = plugin not installed), `ITEM_NOT_FOUND` (no such ID in the source), and `ITEM_REF_PARSE_FAILED` (malformed format). On the server-owner side, see [Item Definition §5.1](../02-server-guide/item-definition.md#51-外部物品源引用craftengine--itemsadder--nexo-等).
...
So: `qi:` prefix resolution depends on CoreLib; reverse lookup / direct generation depend only on QI. For onboarding routes see [API Overview](./api-overview.md#6-接入路线选择).
...
For the full explanation see [Attributes & Numbers](../02-server-guide/attributes-numbers.md).
...
For the linking mechanism see [Integration → QinhSkills](./integration.md#2-qinhskills-集成); for onboarding steps see [Integration in Practice](./integration-howto.md#二qinhskills技能引擎).
...
See [Gem Sockets](../02-server-guide/gem-sockets.md) and [Integration → Legendinlay](./integration.md#3-legendinlay-集成宝石孔后端-a).
...
For startup-order details see [Config Files → Startup Order](../05-reference/config.md#启动顺序参考).

---

## Next steps

- [Integration (architecture)](./integration.md): registry / bridges
- [Integration in Practice](./integration-howto.md): per-plugin onboarding
- [API Cookbook](./api-recipes.md): finished code
