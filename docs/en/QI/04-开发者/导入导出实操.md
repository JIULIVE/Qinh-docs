...
> Belongs to: [Developer](./api-overview.md)　·　Related: [Import/Export (API)](./import-export.md) · [Commands](../05-reference/commands.md)

[Import/Export](./import-export.md) covers the API and formats; this page covers the **complete steps for migrating from MMOItems / NeigeItems to QI**, field mapping references, and how to read dry-run reports.
...
> The three core items go into base_values (managed by the base editor); the rest go into `providers.ap`. Ratio types are automatically ÷100. Everything is ultimately handed to AttributePlus, so remember to configure [attribute-mapping](../02-server-guide/attributes-numbers.md#3-配置属性映射) properly.
...
> `level` / `permission` / `world` are judged natively by QI; `class` / faction etc. are delegated to [`QinhItemUseCheckEvent`](./events.md), where you need to implement the class check.
...
For field details see [Import/Export (API)](./import-export.md).
...
- [Diagnostics & Troubleshooting Compendium](../05-reference/diagnostics.md)
- [Events](./events.md): QinhItemUseCheckEvent
