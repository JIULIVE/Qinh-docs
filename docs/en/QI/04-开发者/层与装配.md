...
> Belongs to: [Developer](./api-overview.md)　·　Related: [Core Concepts](../01-getting-started/core-concepts.md) · [API Reference](./api-reference.md)
...
> To change attributes, go through a [Provider](./providers-bridges.md), not a layer patch.
...
For configuration see [config.yml → write-domains](../05-reference/config.md#写域隔离-write-domains). Overstepping writes raise an error when `strict: true`.
...
For the server-owner-facing explanation see [Fragments & Definitions → Library Manifest](../02-server-guide/fragments-templates.md#8-库清单library-manifest).
