...
> Belongs to: [Developer](API概览.md)　·　Related: [Provider and Bridge](Provider与桥.md) · [Gem Sockets](../02-服主指南/宝石孔.md)
...
The `qinhskills:cast` handler in item actions hands skill casting over to [QinhSkills](../02-服主指南/动作系统/处理器.md#生态处理器其它插件注册) (QS).
...
The `lore` in the socket catalog `integrations/legendinlay_sockets.yml` must match the LI side verbatim (LI identifies sockets by Lore); see [Gem Sockets](../02-服主指南/宝石孔.md#4-孔-lore-渲染优先级).
...
> Note: **Prefix resolution (`qi:` / `qinhitems:`) lives in CoreLib**, while item reverse-lookup (item → id) lives in QI. See [API Overview → Integration Route](API概览.md#6-接入路线选择) for details.

---

## Next Steps

- [Provider and Bridge](Provider与桥.md): bridge abstraction details
