...
> Belongs to: [Developer](./api-overview.md)　·　Related: [Integration (Architecture)](./integration.md) · [Attributes & Values](../02-server-guide/attributes-numbers.md) · [Gem Sockets](../02-server-guide/gem-sockets.md)

[Integration](./integration.md) covers the architecture; this page covers the **complete steps from installation to integration testing**—for server admins and developers to wire up each external plugin step by step.
...
For full details see [Attributes & Values](../02-server-guide/attributes-numbers.md).
...
For the linking mechanism see [Integration → QinhSkills](./integration.md#2-qinhskills-集成).
...
For details see [Gem Sockets](../02-server-guide/gem-sockets.md) and [Integration → Legendinlay](./integration.md#3-legendinlay-集成宝石孔后端-a).
...
Shares the `LegendinlayProviderParser` parsing format. For details see [Integration → MagicGem](./integration.md#4-magicgem-集成宝石孔后端-b).
...
For the full placeholder list see [Placeholders](../05-reference/placeholders.md).
...
For a comparison of integration routes see [API Overview](./api-overview.md#6-接入路线选择).

---

## Next Steps

- [Integration (Architecture)](./integration.md): bridges / registry internals
- [API Recipes](./api-recipes.md): complete code for market / mail / lottery, etc.
- [Provider & Bridge](./providers-bridges.md)
