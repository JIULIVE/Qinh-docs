...
> Belongs to: [Server Owner Guide](./item-definition.md)　·　Related: [Item Types](./item-types.md) · [Attributes & Numbers](./attributes-numbers.md) · [Action System](./action-system/overview.md)
...
> 📦 Want a ready-made recipe to copy? See [Item Example Library (~140 built-in examples)](./item-cookbook.md).　🩺 Got an error? See [Validation Error Quick Reference](../05-reference/validation-errors.md).
...
| **`type`** | String | inferred from file name / `misc` | **Item type**, see [Item Types](./item-types.md) |
...
| **`tier`** | String? | null | **Quality** (uppercase, e.g. `EPIC`), see [Quality](./quality-display.md) |
| **`providers`** | Map | empty | **Data providers** (attributes `ap`, binding effects, etc.), see §4 below |
| **`variables`** | Map\<String,String\> | empty | **Variables**, see [Variables](./variables.md) |
| `base_values` | Map\<String,Double\> | empty | Base values (compiled into `ap`), see [Attributes](./attributes-numbers.md) |
| `enchantments` | Map\<String,Int\> | empty | Enchantments: `key=level` |
| `enchant_max_total_level` | Int? | null | Total enchantment level cap for this item, see [Enchantment Cap](./enchant-caps.md) |
| `gem-sockets` | List\<String\> | empty | List of gem socket type IDs, see [Gem Sockets](./gem-sockets.md) |
...
| `sections` | List\<String\> | empty | Referenced [Section](./sections.md) IDs |
| `affixes` | List\<String\> | empty | Referenced [affix](./affixes.md) IDs |
| `fragments` | List\<String\> | empty | Referenced [Fragment](./fragments-templates.md) IDs |
| `resource_pack` | section | null | [Resource pack / custom model](./resource-pack.md) |
| **`options`** | section | — | **Item options** (see §3 below) |
| `actions` | section | — | **Actions / triggers** (see [Action System](./action-system/overview.md)) |
...
Written in the `options.restrictions` list; validated when equipping / using. If unmet, the item's lore is marked red and use is blocked (via [`canUse`](../04-developer/api-reference.md) / `QinhItemUseCheckEvent`):
...
See [API Reference → canUse](../04-developer/api-reference.md#使用条件检查) for details.
...
- **`ap`** — combat attributes. See [Attributes & Numbers](./attributes-numbers.md) for details.
- **`perm_effects`** — persistent potion effects while worn: `{"effect_name":amplifier}`, 0 = level I.
- **`legendinlay` / `magicgem`** — gem socket payloads, usually auto-synced and generated by the GUI / `gem-sockets`, see [Gem Sockets](./gem-sockets.md) for details.

> A Provider can also be written in a multi-carrier-key sub-section form; for developer details see [Providers & Bridges](../04-developer/providers-bridges.md). Allowed carrier keys: `value` / `raw` / `data` / `payload`.
...
- **When referencing an external source, you usually don't need to also configure `resource_pack.custom_model_data`**—the external item already comes with its own model. For the CMD / resource-pack route, see [Resource Packs & Custom Models](./resource-pack.md).
...
> These prefixes are provided uniformly by QinhCoreLib and are shared across the whole ecosystem (QI / QinhSkills, etc.). For the full explanation, see [Cross-Module Integration → QI ↔ CoreLib](../04-developer/cross-module-integration.md#二qi--corelib).
...
For the validation rules (errors you may hit when saving), see [Resource Packs](./resource-pack.md#校验约束) and the developer-oriented constraint-system explanation.

---

## Next Steps

- See which types are available and what each can add → [Item Types](./item-types.md)
- Add attributes to an item → [Attributes & Numbers](./attributes-numbers.md)
- Give an item active skills → [Action System](./action-system/overview.md)
