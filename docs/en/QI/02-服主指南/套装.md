...
> Belongs to: [Server Owner Guide](物品定义.md)　·　Related: [Attributes & Values](属性与数值.md) · [Action System](动作系统/概览.md)
...
- Set attributes go through an independent source of the [Attribute System](属性与数值.md); set perm effects go through `PermEffectSync`.
...
The `handler` + `payload` in a skill's `refs` are dispatched by QI (e.g. `qinhskills:cast` is handed to [QinhSkills](../04-开发者/集成.md)). The `cooldown` supports `5s` / `200ms` / `3t` (ticks).
...
For editing sets via GUI, see [Set Editor](../03-编辑器GUI/套装编辑器.md).

---

## Next Steps

- [Action System](动作系统/概览.md): handlers used by set skills
- [Attributes & Values](属性与数值.md): how set attributes are applied
- [Set Editor](../03-编辑器GUI/套装编辑器.md): orchestrate sets via GUI
