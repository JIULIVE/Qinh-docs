...
> Belongs to: [Server Owner Guide](物品定义.md)　·　Related: [Section](段.md) · [Random Generation](随机生成.md) · [Quality and Display](品质与显示.md)

An affix is a **reusable modifier package**: it adds a prefix / suffix to an item's name and appends a line of Lore. Combined with the pool mechanism of [Sections](段.md), it can implement random affixes.

---

## 1. What Is an Affix

An affix (`QinhAffix`) is essentially a named container carrying "prefix / suffix / Lore + arbitrary metadata". Items reference it through the `affixes:` list. Affixes are usually used together with [Sections](段.md) and [Random Generation](随机生成.md).
...
- The `lore` is appended as a line of Lore (placed in the "Section Lore" section of the [structured Lore](品质与显示.md#5-lore-段顺序从上到下)).
...
In short: **an affix is the "ingredient", a section is "how the ingredient is drawn"**. For random prefixes/suffixes, use a section's `weight_join`/`quality_pool` pool; to fixedly attach one affix, reference it directly with `affixes:`. See [Section](段.md) for details.
...
Management commands: `/qi affixes`, `/qi affixes summary`, `/qi affixes categories`, `/qi affixes formats`, see [Commands](../05-参考/命令.md).

---

## Next Steps

- [Section](段.md): weighted random pools, quality pools
- [Random Generation](随机生成.md): draw affixes from a pool by capacity
