...
> Belongs to: [Server Owner Guide](./item-definition.md)　·　Related: [Quality & Display](./quality-display.md) · [Section](./sections.md) · [Affix](./affixes.md)
...
All parameters can be omitted (if left blank, a random Tier and the default pool are used). See [Commands](../05-reference/commands.md).
...
The capacity parameters come from the Tier's `generation.capacity` (see [Quality](./quality-display.md#3-tier-字段表)):
...
Each pool (a [Section](./sections.md)) is rolled according to its type:
...
The corresponding command is `/qi refresh` (and `all` / `inventory` / `armor` / `equipment` / `hand`), see [Commands](../05-reference/commands.md). Refresh = call `assembly().rebuild()` + `variables().refresh()` on each QI item, preserving the instance / seed and only updating the display.

---

## Next Steps

- [Quality & Display](./quality-display.md): configure the Tier's chance / capacity
- [Section](./sections.md) / [Affix](./affixes.md): configure the affix pools
