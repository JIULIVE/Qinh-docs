...
> Belongs to: [Server Owner Guide](./item-definition.md) → [Item Example Library](./item-cookbook.md) · Misc　·　Related: [Item Types](./item-types.md) · [Action System](./action-system/overview.md)
...
Economy currency category. The base materials are mostly vanilla precious-metal materials, stackable; interactive currencies use the [trigger](./action-system/triggers.md) `left_click` combined with `consume` to deduct themselves and complete the exchange.
...
- [trigger](./action-system/triggers.md) `left_click` + `cooldown: 3s` + `consume: self:1`: deducts 1 each time with a 3-second cooldown. The [handler](./action-system/handlers.md) chain runs in order: `qi:command` (tops up the economy plugin, with `{player}` replaced by the player's name), `qi:action_bar` prompt, and `qi:sound` feedback.

---

## Token token.yml

Exchange-voucher category. All are triggered for use via the [trigger](./action-system/triggers.md) `left_click`, paired with `consume: self:1` to deduct the voucher; the exchange result can be a command reward, or `qi:give_item` to directly hand out an item.
...
- [trigger](./action-system/triggers.md) `left_click` + `consume: self:1`; the [handler](./action-system/handlers.md) `qi:command` invokes the kit command, then provides feedback via sound and action_bar.
...
- The [handler](./action-system/handlers.md) `qi:give_item`'s payload is another QI item ID (`demo_void_reaver`, a weapon from the Equipment chapter), which goes straight into the inventory after the voucher is deducted; then `qi:title` (a full-screen title) and a sound reinforce the ceremony of the exchange.

---

## Trophy trophy.yml

Commemorative / loot category. Most are pure-appearance display pieces (`material` + `display_name` + `glow`); if an interactive memento is needed, you can attach the [trigger](./action-system/triggers.md) `left_click`.
...
- No `consume` (showing off does not consume it); the [handler](./action-system/handlers.md) `qi:console_command` executes `broadcast` with console permissions to announce server-wide, with `{player}` replaced by the holder's name.

---

## Skull skull.yml

Decorative skull category. Most are pure appearance; player heads specify the skin owner via `options.skull_owner`. You can attach [triggers](./action-system/triggers.md) such as `left_click` / `shift_left_click`.
...
- **skull_owner mechanism**: `player_head` renders as the `Herobrine` skin head. The [trigger](./action-system/triggers.md) `shift_left_click` (sneak + left-click) + `consume: self:1` implements the "sacrifice"; the [handler](./action-system/handlers.md) `qi:give_item` hands out the `demo_creeper_trophy` above, forming a skull-swap chain.

---

## Prop prop.yml

General prop category. Relies on the [trigger](./action-system/triggers.md) `left_click` as a "use" gesture to trigger effects like `qi:title` / `qi:sound` / `qi:command`.
...
- `cooldown: 2m` (minute-level cooldown) + `consume: self:1` (single-use); the [handler](./action-system/handlers.md) `qi:command` executes `spawn` as the player to return to the spawn point.
...
Mount-related category. Appearance pieces are saddles / leads; summoning types use a spawn-egg base material + a [trigger](./action-system/triggers.md) that fires the `summon` command via `qi:console_command` to spawn the mount entity.
...
- **Summon mechanism**: QI itself has no built-in mob spawning; instead it relies on the [handler](./action-system/handlers.md) `qi:console_command` to execute the vanilla `summon minecraft:horse ~ ~ ~ {Tame:1b}` — `~ ~ ~` is the player's coordinates, and the NBT `{Tame:1b}` makes the horse already tamed on spawn. `consume: self:1` makes the summon egg single-use.
...
- **Summon mechanism**: the [handler](./action-system/handlers.md) `qi:console_command` summons `minecraft:skeleton_horse` (a skeleton horse, likewise tamed with `{Tame:1b}`). `options.bind_on_acquire: true` binds it on acquisition, and `restrictions: level:30` prevents use below level 30; the [trigger](./action-system/triggers.md) is `shift_left_click` (sneak + left-click). This example carries no `consume`, so it can be summoned repeatedly (subject to the 10-second cooldown).

---

## Next Steps

- [Example - Equipment](./cookbook-equipment.md)
- [Example - Accessories & Consumables](./cookbook-accessories-consumables.md)
- [Item Example Library Overview](./item-cookbook.md)
