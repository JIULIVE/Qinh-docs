...
> Belongs to: [Server Owner Guide](物品定义.md) → [Item Example Library](物品示例库.md) · Accessories & Consumables · Related: [Item Types](物品类型.md) · [Gem Slots](宝石孔.md) · [Action System](动作系统/概览.md)
...
- No `actions`; the attribute is settled passively by `providers.ap`, no trigger involved; for the attribute JSON syntax see [Item Definition](物品定义.md).
...
- The `equip` [trigger](动作系统/触发器.md) is the feedback entry point for the accessory's passive; two [handlers](动作系统/处理器.md): `qi:action_bar` + `qi:sound`.
...
- The `equip` / `unequip` [triggers](动作系统/触发器.md) each carry feedback; `options.restrictions: ["level:15"]` is a usage restriction — see [Cooldown/Cost/Condition](动作系统/冷却消耗条件.md).
...
- `equip` [trigger](动作系统/触发器.md) + amethyst chime sound.
...
- `options.bind_on_acquire: true` binds on acquire; `equip` / `unequip` dual [triggers](动作系统/触发器.md).
...
- Base material is `nether_star`; the `equip` [trigger](动作系统/触发器.md) is paired with the `qi:title` [handler](动作系统/处理器.md).
...
- Has both `bind_on_acquire` and `restrictions: ["level:20"]`; for restriction semantics see [Cooldown/Cost/Condition](动作系统/冷却消耗条件.md).
...
- Three-dimensional attributes (attack/defense/health) + `equip` [trigger](动作系统/触发器.md).
...
- `bind_on_acquire` + `equip` / `unequip` dual [triggers](动作系统/触发器.md).
...
The food-specific fields `options.food.nutrition` / `saturation` customize hunger restoration — see [Item Definition](物品定义.md) for details.
...
- The `consume` [trigger](动作系统/触发器.md) only works for edible materials; `options.food` customizes `nutrition: 8` / `saturation: 12.8`.
...
- `cooldown: 30s` prevents buff spamming; `qi:console_command` runs `effect give {player} ...` with console permissions; for cooldown semantics see [Cooldown/Cost/Condition](动作系统/冷却消耗条件.md).
...
- `consume: ["self:1"]` for one-time consumption + `cooldown: 3s`, see [Cooldown/Cost/Condition](动作系统/冷却消耗条件.md); `qi:command` executes `spawn` as the player.
...
- Same structure as the Return Scroll, `cooldown: 5s` + `self:1`, with three feedback [handlers](动作系统/处理器.md).
...
- The `place_block` [trigger](动作系统/触发器.md) fires when the seed is scattered onto farmland; `cooldown: 2s`.
...
- `left_click` [trigger](动作系统/触发器.md) + `cooldown: 3s`, no consumption.
...
- `consume: ["self:3"]` eats 3 of this item at once, and the `qi:give_item` [handler](动作系统/处理器.md) produces `demo_iron_fragment`; for consumption semantics see [Cooldown/Cost/Condition](动作系统/冷却消耗条件.md).
...
- `break_block` [trigger](动作系统/触发器.md) + `cooldown: 2s`.
...
- `break_block` (`cooldown: 1s`) and `left_click` (`cooldown: 500ms`) are two independent [triggers](动作系统/触发器.md), each with a different cooldown; `restrictions: ["level:10"]`.
...
- `fish` [trigger](动作系统/触发器.md) + `cooldown: 2s`.
...
- `fish` trigger + `cooldown: 5s`; the `qi:give_item` [handler](动作系统/处理器.md) gives out `demo_angler_rod`.
...
- [Example - Equipment](示例-装备篇.md) — Weapons, armor, and their [Gem Slots](宝石孔.md) and combat trigger examples
- [Example - Miscellaneous](示例-杂项篇.md) — Other item types and advanced action combinations
- [Item Example Library Overview](物品示例库.md) — Return to the example library index page
