...
> Belongs to: [Server Owner Guide](./item-definition.md)　·　Related: [Variables](./variables.md) · [Sections](./sections.md) · [Sets](./sets.md)
...
> Variables only affect the **display**; the actual attribute values live in `providers.ap`. To keep the display consistent with the attributes, you must coordinate the two yourself, or use the capacity / affix mechanisms of [Random Generation](./random-generation.md). See [Variables](./variables.md) for the variable mechanism.
...
Render order: quality prefix + affix prefix → item name → affix suffix + guardian suffix; the lore accumulates the description lines of each section / affix. See [Sections](./sections.md) for the section mechanism, [Affixes](./affixes.md) for affixes, and [Quality and Display](./quality-display.md#5-lore-段顺序从上到下) for display order.
...
The item references `quality_damage_pool`; during random generation it picks the matching sub-pool by the item's tier and draws from it. See [Random Generation](./random-generation.md#5-词缀池抽取) for random generation.
...
Event-type abilities **auto-trigger** once enough pieces are worn (independent of which item is held). See [Sets](./sets.md) for the set mechanism; skills are handled by [QinhSkills](../04-developer/integration.md).
...
An enhancement system can change `star` at runtime via the [Layer API / Variable API](../04-developer/api-recipes.md#配方-4强化--镶嵌系统写层补丁).
...
See [Soul Binding](./soulbinding.md) for binding, [Item Definition](./item-definition.md#restrictions-限制语法) for restrictions, and [Action System](./action-system/overview.md) for actions.

---

## Next Steps

- [Variables](./variables.md) / [Sections](./sections.md) / [Sets](./sets.md): underlying mechanisms
- [Random Generation](./random-generation.md): affix pools + capacity
- [Item Example Library](./item-cookbook.md): built-in examples
