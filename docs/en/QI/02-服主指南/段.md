...
During random generation, `quality_pool` will further treat the matched sub-section as a `weight_join` pool and draw from it. See [Random Generation](./random-generation.md) for details.
...
4. Then process the [affixes](./affixes.md) referenced by `affixes:`.

Returns `RenderResult(prefix, suffix, loreLines, warnings)`, merged into the "Section Lore" segment of the [structured Lore](./quality-display.md#5-lore-段顺序从上到下).
...
Admin commands: `/qi sections`, `/qi sections summary`, `/qi sections formats`, `/qi sections format`. See [Commands](../05-reference/commands.md).
...
- [Random Generation](./random-generation.md): hook section / affix pools into random drops
- [Affixes](./affixes.md): define named affixes
