...
> Belongs to: [Action System](./overview.md)　·　Related: [Handler Reference](./handlers.md) · [Triggers in Practice](./triggers-hands-on.md)

The [Handler Reference](./handlers.md) is a quick lookup table; this page **expands each built-in handler one by one**: the meaning of each payload field, the various ways to write it, and common pitfalls.
...
- Requires `combat.enabled: true`. See [Attributes and Stats](../attributes-numbers.md).
...
- In a set's `abilities`, the handler must use the **map form** (with a colon), see [Sets](../sets.md).
- See [Integration](../../04-developer/integration.md).
...
[Develop a custom handler](../../04-developer/handler-development.md): implement `QinhActionHandler`, write the logic in code, and reference it from YAML. This is the only proper way to bring if/switch/state checks into an action (YAML cannot express logic).

---

## Next Steps

- [Handler Reference](./handlers.md): quick lookup table
- [Triggers in Practice](./triggers-hands-on.md): triggers + handler combinations
- [Action Handler Development](../../04-developer/handler-development.md)
