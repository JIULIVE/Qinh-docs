...
> Belongs to: [Action System](概览.md)　·　Related: [Triggers](触发器.md) · [Action Handler Development](../../04-开发者/动作处理器开发.md)
...
> 🔎 Want **per-field details + multiple writing variants + common pitfalls** for each handler? See [Action Handler Details](处理器详解.md).
...
**qinhskills:cast** — hands skill casting over to [QinhSkills](../../04-开发者/集成.md):
```yaml
- handler: qinhskills:cast
  payload: '{"skill":"fireball","level":1}'
```

**combat:swing** — a swing that works with the [Attribute System](../属性与数值.md):
```yaml
...
A handler's payload has two modes (determined by its [payload schema](../../04-开发者/动作处理器开发.md#payload-schema)):
...
The GUI editor automatically generates a form based on the schema ([Action Editor](../../03-编辑器GUI/动作编辑器.md)).

---

## Developing Custom Handlers

Need an effect beyond the built-in handlers? Implement the `QinhActionHandler` interface and register it. This is the **only proper way** to bring complex logic (if/switch/state) into the action system (you cannot write logic in YAML). See [Action Handler Development](../../04-开发者/动作处理器开发.md) for details.
...
- [Cooldown / Cost / Conditions / Combos](冷却消耗条件.md)
- [Action Handler Development](../../04-开发者/动作处理器开发.md): custom handlers + payload schema
