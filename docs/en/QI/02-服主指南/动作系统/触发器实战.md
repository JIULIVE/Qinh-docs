...
> Belongs to: [Action System](概览.md)　·　Related: [Trigger Atom Reference](触发器.md) · [Handlers](处理器.md)

The [Trigger Atom Reference](触发器.md) lists every atom; this page gives **one minimal, ready-to-run YAML per trigger category**—just copy and go. Every example shows only the `actions` block; merge it into your item definition (note that `actions` is at the same level as `material`).
...
For condition / cost / cooldown syntax, see [Cooldown / Cost / Condition](冷却消耗条件.md).

---

## Next Steps

- [Trigger Atom Reference](触发器.md): the full atom list
- [Handlers](处理器.md): the payload for each handler
- [Cooldown / Cost / Condition](冷却消耗条件.md)
