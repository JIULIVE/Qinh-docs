...
> Belongs to: [Server Owner Guide](../物品定义.md) → Action System　·　Subpages: [Triggers](触发器.md) · [Handlers](处理器.md) · [Cooldown/Cost/Condition](冷却消耗条件.md)
...
For event details, see [Events](../../04-开发者/事件.md).
...
**What if you need complex logic?** Have a developer implement an [action handler](../../04-开发者/动作处理器开发.md), writing the if/switch/state checks into Kotlin/Java, while the YAML only references it and passes a payload.
...
- **Trigger**: 100+ atoms. Click-type (left/right/shift), sneak (single click / double click / sequence), movement (sprint / glide / jump), combat (hit / take damage / kill / crit), container / crafting, entity interaction, environment, server events, `tick` periodic. For the full list, see [Triggers](触发器.md).
- **Handler**: 10 built-in (`qi:message` / `qi:action_bar` / `qi:title` / `qi:subtitle` / `qi:sound` / `qi:bossbar` / `qi:command` / `qi:console_command` / `qi:give_item` / `qi:close_inventory`) + ecosystem (`qinhskills:cast` / `combat:swing`). For each one's payload format, see [Handlers](处理器.md).
...
See [Commands](../../05-参考/命令.md).

---

## Subpage navigation

- [Complete Trigger Atom Reference](触发器.md) —— all 100+ trigger atoms listed one by one
- [Complete Action Handler Reference](处理器.md) —— payload format for each handler
- [Cooldown / Cost / Condition / Combo](冷却消耗条件.md) —— syntax details
- Develop custom handlers → [Action Handler Development](../../04-开发者/动作处理器开发.md)
