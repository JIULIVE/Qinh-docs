...
> Belongs to: [Server Admin Guide](./item-definition.md) → [Item Example Library](./item-cookbook.md) · Equipment　·　Related: [Item Types](./item-types.md) · [Attributes & Values](./attributes-numbers.md) · [Action System](./action-system/overview.md)
...
Values are applied by `providers.ap.value` (a chunk of attribute JSON) through [AttributePlus](./attributes-numbers.md); QI itself does not bundle values. See [Attributes & Values](./attributes-numbers.md) for details.
...
- Entry-level baseline: `providers.ap.value` provides `defense` / `health` ([Attributes & Values](./attributes-numbers.md)), `unbreakable` prevents durability loss. With no `tier`, it defaults to COMMON ([Quality & Display](./quality-display.md)).
...
- `tier: EPIC` combined with `glow: true` forms an epic appearance ([Quality & Display](./quality-display.md)).
...
- The first armor with actions: the `equip` [trigger](./action-system/triggers.md) drives the `qi:action_bar` + `qi:sound` [handlers](./action-system/handlers.md); the sound format in `payload` is `sound key;volume;pitch`.
...
- Entry-level baseline, only `defense:4` ([Attributes & Values](./attributes-numbers.md)).
...
- `tier: RARE` + `glow` ([Quality & Display](./quality-display.md)).
...
- `equip` [trigger](./action-system/triggers.md) → `qi:action_bar` + `qi:sound` (netherite equip sound effect) [handlers](./action-system/handlers.md).
...
- Demonstrates multiple triggers on one item: `equip` has `cooldown: 2s` and uses `qi:title` (format `title||subtitle||fade-in||stay||fade-out`); `unequip` uses `qi:message` ([trigger](./action-system/triggers.md) · [handler](./action-system/handlers.md)).
...
- Entry-level baseline ([Attributes & Values](./attributes-numbers.md)).
...
- `tier: EPIC` + `glow` ([Quality & Display](./quality-display.md)).
...
- `equip` [trigger](./action-system/triggers.md) → `qi:title` + beacon activation sound [handlers](./action-system/handlers.md).
...
- `bind_on_acquire: true` [Soul Binds](./soulbinding.md) on acquire; `restrictions: level:20` restricts use to level 20. `equip` uses `qi:bossbar` (format `text;progress;color;style;duration in ticks`), and `unequip` prompts removal ([trigger](./action-system/triggers.md) · [handler](./action-system/handlers.md)).
...
- `equip` [trigger](./action-system/triggers.md) → `qi:title` + equip sound [handlers](./action-system/handlers.md).
...
- `bind_on_acquire` [Soul Bind](./soulbinding.md) + `restrictions: level:20`; `equip` / `unequip` dual [triggers](./action-system/triggers.md) each fire `qi:action_bar` ([handler](./action-system/handlers.md)).
...
- Introduces the `movement_speed` attribute ([Attributes & Values](./attributes-numbers.md)), `tier: RARE` + `glow`.
...
- `equip` / `unequip` dual [triggers](./action-system/triggers.md) each fire `qi:action_bar`, with equip adding a sound effect ([handlers](./action-system/handlers.md)).
...
- Highest movement speed `0.12`, `bind_on_acquire` [Soul Bind](./soulbinding.md) + `restrictions: level:20`; the `equip` [trigger](./action-system/triggers.md) fires `qi:title` + dragon wing-flap sound ([handlers](./action-system/handlers.md)).
...
- Entry-level baseline, `attack_damage:6` ([Attributes & Values](./attributes-numbers.md)).
...
- `left_click` [trigger](./action-system/triggers.md) (the most reliable gesture) + `cooldown: 2s`, with the `crit_rate` critical hit rate attribute ([Attributes & Values](./attributes-numbers.md)).
...
- `on_hit` [trigger](./action-system/triggers.md) (when the arrow hits an entity), `bind_on_acquire` [Soul Bind](./soulbinding.md) + `restrictions: level:20`.
...
- `left_click` [trigger](./action-system/triggers.md) + `cooldown: 2s`, includes `crit_rate` ([Attributes & Values](./attributes-numbers.md)).
...
- `on_hit` [trigger](./action-system/triggers.md), `bind_on_acquire` [Soul Bind](./soulbinding.md) + `restrictions: level:20`.
...
- `left_click` [trigger](./action-system/triggers.md) + `cooldown: 3s`, `qi:title` + riptide sound [handlers](./action-system/handlers.md).
...
- `material: spectral_arrow` + `glow` ([Quality & Display](./quality-display.md)).
...
- `on_hit` [trigger](./action-system/triggers.md) + `cooldown: 2s`, no `providers`, relying solely on action effects ([handlers](./action-system/handlers.md)).
...
- The only arrow with `providers` (`attack_damage:6`, [Attributes & Values](./attributes-numbers.md)), `restrictions: level:15`, `on_hit` fires `qi:title` ([trigger](./action-system/triggers.md) · [handler](./action-system/handlers.md)).
...
- `equip` [trigger](./action-system/triggers.md) → `qi:action_bar` + shield block sound [handlers](./action-system/handlers.md).
...
- Demonstrates the `on_damage` when-struck [trigger](./action-system/triggers.md) + `cooldown: 2s`, `bind_on_acquire` [Soul Bind](./soulbinding.md) + `restrictions: level:20`.
...
- Entry-level baseline, only `glow`, no `unbreakable` ([Attributes & Values](./attributes-numbers.md)).
...
- `tier: LEGENDARY` high-end stats with no actions ([Quality & Display](./quality-display.md)).
...
- Three [handlers](./action-system/handlers.md) chained together: `qi:title` + `qi:sound` + `qi:bossbar`, `left_click` [trigger](./action-system/triggers.md) + `cooldown: 3s`.
...
- The only equipment that demonstrates `consume` (`item:redstone:1` consumes 1 redstone as a mana stone on release), paired with `cooldown: 2s` + `restrictions: level:15`, including the `qi:subtitle` [handler](./action-system/handlers.md) ([trigger](./action-system/triggers.md)).
...
- Entry-level baseline, not even an `options` block ([Attributes & Values](./attributes-numbers.md)).
...
- `equip` / `unequip` dual [triggers](./action-system/triggers.md) demonstrate barding / unbarding prompts ([handlers](./action-system/handlers.md)).
...
- Dual [triggers](./action-system/triggers.md): `left_click` (wipe, with `cooldown: 3s`) + `equip` (barding horn). The `max_health` attribute ([Attributes & Values](./attributes-numbers.md)), `bind_on_acquire` [Soul Binds](./soulbinding.md) on acquire (no `restrictions` set here).

---

## Next Steps

- [Examples - Accessories & Consumables](./cookbook-accessories-consumables.md)——examples of accessories, consumables, dishes, and other types
- [Examples - Miscellaneous](./cookbook-misc.md)——examples of tools, blocks, and miscellaneous types
- [Item Example Library Overview](./item-cookbook.md)——return to the example library directory
