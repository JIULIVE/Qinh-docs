...
> **How is capacity computed into a slot count?** See [Random Generation → Capacity Calculation](随机生成.md#3-容量计算). Formula: `base + scale×rand + spread×rand + maxSpread×rand`, floored, and not less than 0.
...
| 5 | **Section Lore** | [Section](段.md) rendering | Lines produced by the affix pool |
...
| 9 | **Set Lore** | [Set](套装.md) rendering | Set piece count / bonuses |
...
| 12 | **Soul Binding** | Instance data | `xxx's item`, see [Soul Binding](灵魂绑定.md) |
...
2. Then the **prefixes / suffixes** produced by [Sections](段.md) / [Affixes](词缀.md) are applied.
...
- Make quality participate in random drops → [Random Generation](随机生成.md)
- Add prefixes/suffixes to names with affix pools → [Affixes](词缀.md) / [Sections](段.md)
- Configure attribute values → [Attributes and Values](属性与数值.md)
