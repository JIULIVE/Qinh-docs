...
> Belongs to: [Editor GUI](./overview.md)　·　Related: [Item Editor](./item-editor.md) · [Slot Cheat Sheet](./slot-reference.md)

Subpages like [Item Editor](./item-editor.md) explain "what the interface is", and the [Slot Cheat Sheet](./slot-reference.md) tells you "which slot to click". This page gives the **complete step-by-step workflows for common tasks**—just follow along to get them done.
...
For attribute mechanics see [Attributes & Values](../02-server-guide/attributes-numbers.md). AttributePlus must be installed for them to actually apply.
...
> Tip: In the action editor, the handler list supports left-click to move up, right-click to delete, Shift+right-click to edit the payload. For slots see [Slot Cheat Sheet](./slot-reference.md#动作编辑器-actionrefseditiongui).
...
This is equivalent to adding `qinhskills:cast` to the refs of `left_click`. Requires QinhSkills, see [Integration](../04-developer/integration.md).
...
For set mechanics see [Sets](../02-server-guide/sets.md), and for button slots see [Slot Cheat Sheet](./slot-reference.md#套装-sethubgui).
...
For socket type definitions see [Gem Sockets](../02-server-guide/gem-sockets.md).
...
Or hold the item and run `/qi soulbind [level]` to bind it directly to yourself. See [Soulbinding](../02-server-guide/soulbinding.md).
...
- If there are **validation errors** on save, you'll be prompted and the save rejected (see [Validation Error Cheat Sheet](../05-reference/validation-errors.md)).
...
- [Item Editor](./item-editor.md) / [Action Editor](./action-editor.md) / [Set Editor](./set-editor.md)
- [Slot Cheat Sheet](./slot-reference.md): exact slot numbers
