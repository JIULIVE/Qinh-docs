...
> Belongs to: [Editor GUI](./overview.md)　·　Related: [Fragments & Templates](../02-server-guide/fragments-templates.md) · [Resource Pack](../02-server-guide/resource-pack.md)
...
- **Lore / Fragments** (FragmentSelectionGui + LoreBlockGui):
  - Paged management of referenced [fragment](../02-server-guide/fragments-templates.md) IDs.
  - Add / edit / delete Lore lines (chat input).
- **Variables** (VariableDeclarationGui):
  - Paged management of [variable](../02-server-guide/variables.md) entries (key=value).
  - Add: enter key → enter value.
  - Shift + right-click to edit value; right-click to delete.

### Skills Tab

Shows the action table + trigger count + skill binding overview. Lets you jump to the [Action / Skill editor](./action-editor.md).
...
Edits the item's [custom model](../02-server-guide/resource-pack.md).

- **CMD Editor** (CMDEditorGui): sets `custom_model_data`.
- **Model Selector** (ModelSelectorGui): pick a model from a category in the [model catalog](../02-server-guide/resource-pack.md#3-模型目录编辑器用) in `integrations/resource_models.yml`.

> Note: QI applies [strict isolation validation](../02-server-guide/resource-pack.md#2-校验约束) to resource pack fields — models cannot bind variables / layers / conditions.

---

## 3. Section / Affix Compile (SectionAffixCompileGui)

Used to view / compile [Sections](../02-server-guide/sections.md) and [affixes](../02-server-guide/affixes.md).

---

## 4. Content Pack Editing (ItemPackEditorGui)

Opened with `/qi packs gui`; manages export / import / sync of `.qipack` content packs. See [Import & Export](../04-developer/import-export.md) for the import/export mechanism.
...
For validation rules (errors you may run into), see [Resource Pack → Validation Constraints](../02-server-guide/resource-pack.md#2-校验约束).
...
- [Fragments & Templates](../02-server-guide/fragments-templates.md)
- [Resource Pack](../02-server-guide/resource-pack.md)
- [Import & Export](../04-developer/import-export.md)
