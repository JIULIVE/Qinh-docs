...
> Previous: [Installation & Environment](安装.md)　·　Next: [Core Concepts & Architecture](核心概念.md)
...
> ✅ **Done!** Next you can:
> - Add [random-value variables](../02-服主指南/变量.md)
> - Add [gem sockets](../02-服主指南/宝石孔.md)
> - Bind it into a [set](../02-服主指南/套装.md)
> - Make it [soulbound](../02-服主指南/灵魂绑定.md)
...
Opens the **type browser**. For full navigation see [Editor Overview](../03-编辑器GUI/概览.md).
...
In the item editor, click the matching buttons (see [Item Editor](../03-编辑器GUI/物品编辑器.md)):

- **Material** → type `diamond_sword` in chat
- **Quality** → type `EPIC` in chat
- **Display name** → type `<gold>Trial Blade</gold>` in chat
- **Attributes** → open the [attribute editor](../03-编辑器GUI/物品编辑器.md#属性编辑器), add `attack_damage = 18`
- **Actions/skills** → open the [action editor](../03-编辑器GUI/动作编辑器.md), under the `left_click` tab add the `qi:title` / `qi:sound` handlers
...
- **Attributes don't apply?** You don't have AttributePlus, or `combat.enabled` is off — pure item-library mode. See [Attributes & Numbers](../02-服主指南/属性与数值.md).
- **Action doesn't fire?** Actions load only on `/qi reload`; reload after editing YAML.
- **Quality name not showing?** `tier` must be uppercase (`EPIC`) and exist in `item_tiers.yml`. See [Quality & Display](../02-服主指南/品质与显示.md).
- **`material` errors?** Use a vanilla material name (e.g. `diamond_sword`); anything with `:` or `-` is treated as an [external item-source reference](../02-服主指南/物品定义.md#material-材质).

Next → [Core Concepts & Architecture](核心概念.md)
