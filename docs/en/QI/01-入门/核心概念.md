...
> Previous: [Quick Start in 5 Minutes](快速上手.md)　·　Back to: [Getting Started](概览.md)

Once you understand this page, every later chapter becomes simpler. Here we lay out QI's core terms and data flow. For a complete glossary, see the [Glossary](../05-参考/术语表.md).
...
- **Definition (template)**: the immutable design blueprint compiled from YAML, managed by ID via `QinhItemRegistry`. See [Item Definition](../02-服主指南/物品定义.md).
- **Instance (instance data)**: the mutable state written inside a single ItemStack's NBT. The key is `qinhitems:instance`, and its content is a chunk of YAML. See [Layers & Assembly](../04-开发者/层与装配.md).
...
For the developer API, see [Layers & Assembly](../04-开发者/层与装配.md).
...
QI itself **does not interpret** provider content; it only stores and passes it along. Interpretation is left to the corresponding **Bridge**. See [Provider & Bridge](../04-开发者/Provider与桥.md).
...
Why have this mapping layer? Because QI wants to keep configs portable, while every server may name its attributes differently in AP. See [Attributes & Numbers](../02-服主指南/属性与数值.md).
...
Variables go through a four-stage pipeline of "Roll → Resolve → Render → Conflict resolution". Multiple sources (template / instance / layer / runtime override) are merged by priority and remain traceable. See [Variables](../02-服主指南/变量.md).
...
- **Trigger**: 100+ built-in atoms (click, sneak, move, combat, container, entity interaction…) + sequence combos. See [Triggers](../02-服主指南/动作系统/触发器.md).
- **Handler**: 10 built-in (`qi:message` / `qi:sound` / `qi:title` / `qi:command` / `qi:give_item`…) + ecosystem-registered (`qinhskills:cast` / `combat:swing`). See [Handlers](../02-服主指南/动作系统/处理器.md).

⚠️ **You cannot write logic branches in YAML** (if/else/switch) — this is a deliberate design boundary. For complex logic, please [develop an action handler](../04-开发者/动作处理器开发.md).
...
For example, `weapon` supports SKILL/ATTRIBUTE/GEM_SOCKET/RENDER, while `currency` supports only RENDER. The capability matrix determines whether a given type can have gem sockets or attributes. See [Item Types](../02-服主指南/物品类型.md).
...
|---|---|---|
| `config.yml` | Main config (combat / soulbound / write-domains / attribute-mapping / gem…) | [Configuration Files](../05-参考/配置文件.md) |
| `item_types.yml` | Item types and capabilities | [Item Types](../02-服主指南/物品类型.md) |
| `item_tiers.yml` | Quality / Tier | [Quality & Display](../02-服主指南/品质与显示.md) |
| `enchant_limits.yml` | Enchantment limits | [Enchantment Limits](../02-服主指南/附魔上限.md) |
| `items/*.yml` | Item definitions | [Item Definition](../02-服主指南/物品定义.md) |
| `fragments/*.yml` | Fragments (reusable snippets) | [Fragments & Templates](../02-服主指南/碎片与模板.md) |
| `sections/*.yml` | Sections / affixes / pools | [Sections](../02-服主指南/段.md) / [Affixes](../02-服主指南/词缀.md) |
| `sets/*.yml` | Sets | [Sets](../02-服主指南/套装.md) |
| `integrations/*.yml` | Gem-socket types, bridges, model catalog | [Gem Sockets](../02-服主指南/宝石孔.md) / [Integrations](../04-开发者/集成.md) |
...
- Server owner → [Item Definition](../02-服主指南/物品定义.md)
- Developer → [API Overview](../04-开发者/API概览.md)
