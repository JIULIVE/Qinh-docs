...
> Navigation: [Docs Home](../README.md) · [Contents](../README.md) · [Core Concepts](../01-入门/核心概念.md) · [Module System](../04-开发者/模块系统.md) · [FAQ](FAQ.md) · [Diagnostic Codes](诊断码.md)
...
**QCL (QinhCoreLib)** — The core foundation / platform library of the Qinhuai ecosystem. It implements no gameplay; it provides a unified item source, economy, scripting, database, PDC, placeholders, GUI, action contracts, bridges, and diagnostics. Main class `com.qinhuai.corelib.QinhCoreLib`. See [Overview](../01-入门/概览.md).
...
**Module** — A functional unit in QCL that can be independently `load`/`enable`/`disable`/`unload`-ed, managed by `ModuleManager` in ascending `priority` order. QCL ships 22 built-in core modules. See [Module System](../04-开发者/模块系统.md), [Core Concepts](../01-入门/核心概念.md).
...
**Enabled / Available** — Two layers of state: `enabled` (whether the switch is on + whether loading succeeded) and `available` (whether it can actually work at runtime). See [Diagnostic Codes · Two-Layer State](诊断码.md).

**API boundary / apiJar / public packages / internal packages** — The stable API artifact (apiJar) that QCL publishes externally exposes only the **public packages**; the **internal packages** are implementation details and sub-plugins should not depend on them. See [API Overview](../04-开发者/API概览.md).

---

## 📦 Item Sources

**Item source (ItemSource)** — QCL's unified item-origin abstraction, covering 10 types: vanilla, CraftEngine, MMOItems, NeigeItems, QinhItems, MythicMobs, CustomFishing, MagicGem, ItemsAdder, Nexo. See [Item Source References](../02-服主指南/物品源引用.md).
...
**Reference** — A string identifier pointing to an item, in the form `source:itemId` or `source-itemId`; with no prefix it defaults to vanilla. See [Item Source References](../02-服主指南/物品源引用.md).
...
**ResolveResult** — The structured return of `ItemManagerAPI.resolve(...)` (`success`/`item`/`code`/`message`/`source`/`suggestion`/`traceId`). For error codes see [Diagnostic Codes](诊断码.md).

**ItemModule** — The external item module interface (`build`/`buildWithParams`); a Groovy script can implement it to register a new item source under `item-modules/`. See [Item API](../04-开发者/物品API.md).
...
**Handler (QinhActionHandler)** — An action handler: the callback entry point that handles a category of actions (`handlerId`/`isAvailable`/`dispatch`). The QI→QS contract layer. See [Action & Skill Bridge](../04-开发者/动作与技能桥.md).
...
**Condition / ConditionPipeline / CompositeCondition** — The condition-evaluation interface, pipeline (AND/OR), and composite condition (`LogicOperator`). Operators include `eq`/`neq`/`exists`/`notnull`/`contains`/`var_eq`. See [Conditions & Expressions](../04-开发者/条件与表达式.md).
...
**EconomyBridge** — The unified economy bridge, automatically selecting a source among Vault / ExcellentEconomy / PlayerPoints (`economy.default-provider`). See [Economy API](../04-开发者/经济API.md).
...
**GraalJS / GraalVM** — The JavaScript runtime embedded in QCL; requires Paper/Purpur to pull the runtime library. See [Scripting Getting Started](../02-服主指南/脚本入门.md).
...
**ScriptExecutionResult** — The return of a script execution (`success`/`value`/`code`/`message`); for error codes see [Diagnostic Codes](诊断码.md).

**PDC (PersistentDataContainer)** — The Bukkit persistent data container; QCL's `PdcService` wraps reads/writes by namespace (keys in the form `qinhcorelib:namespace_keyName`). See [Data Storage & Placeholders](../04-开发者/数据存储与占位符.md).
...
**Assembly / DisplayLayer** — The dynamic item display layering system: it splits display (name, lore, etc.) into stackable `DisplayLayer`s, which `apply` to `ItemMeta` in ascending `priority` order. See [Toolkit](../04-开发者/工具集.md).
...
**TaskScheduler** — A synchronous/asynchronous task scheduling wrapper (using `Runnable`/`Supplier` to avoid cross-plugin `LinkageError`), compatible with Folia. Comes with `CooldownManager` (cooldown) and `ThrottledExecutor` (throttling). See [Toolkit](../04-开发者/工具集.md).
...
**CustomBlock / CustomBlockProvider** — The custom block abstraction and its backend provider (built-in `CraftEngineBlockProvider`). See [Custom Blocks](../02-服主指南/自定义方块.md).
...
**Health code (HealthReport)** — Status codes: `OK` (normal), `NO_HOOK` (no soft dependency), `DEGRADED` (degraded but recoverable). See [Diagnostic Codes](诊断码.md).
...
- [Core Concepts](../01-入门/核心概念.md) — Conceptual basics and the startup lifecycle
- [Module System](../04-开发者/模块系统.md) — The module and diagnostic model
- [Diagnostic Codes](诊断码.md) — Code tables for each subsystem
