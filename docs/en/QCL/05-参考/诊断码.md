...
> Navigation: [Documentation Home](../README.md) · [Contents](../README.md) · [Diagnostics & Troubleshooting](../02-服主指南/诊断与排错.md) · [FAQ](FAQ.md) · [Glossary](术语表.md) · [Module System](../04-开发者/模块系统.md)
...
> A bridge can be "enabled but unavailable" (the toggle is on, but the corresponding plugin is not installed → `NO_HOOK`), or "not enabled" (turned off by `modules.*`). When troubleshooting, first figure out which layer you're at. See [Module System](../04-开发者/模块系统.md) for details.
...
| `PARSE_FAILED` | Reference string parse failed | Malformed reference format (empty string, illegal separator, etc.) | Verify the reference format, see [Item Source References](../02-服主指南/物品源引用.md) |
...
> 🔎 The "item reference diagnostic" at the end of `/qcl status` uses `vanilla:stone` as a **self-check**—if it returns `OK`, the item source resolution chain itself is healthy. To troubleshoot a specific reference failure, use `ItemManagerAPI.diagnose(ref, player)` separately. See [Item API](../04-开发者/物品API.md) for details.
...
See [Scripting Quickstart](../02-服主指南/脚本入门.md) and [Script API](../04-开发者/脚本API.md) for details.
...
See [Economy Actions](../02-服主指南/经济动作.md), [Economy Plugins](../03-外部插件对接/经济插件.md), and [Economy API](../04-开发者/经济API.md) for details.
...
Construct with the companion methods `DiagnosticResult.ok(...)` / `DiagnosticResult.fail(...)`. See [Module System · Diagnostic Model](../04-开发者/模块系统.md) for details.
...
- [Diagnostics & Troubleshooting](../02-服主指南/诊断与排错.md) — Line-by-line reading of `/qcl status` and a troubleshooting decision tree
- [FAQ](FAQ.md) — Frequently asked questions
- [Module System · Diagnostic Model](../04-开发者/模块系统.md) — `DiagnosticResult` / `HealthReport` / `BridgeStatus`
