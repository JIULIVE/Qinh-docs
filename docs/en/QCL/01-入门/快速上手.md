> Previous: [Installation](安装.md)　·　Next: [Core Concepts](核心概念.md)
...
> 📌 Prerequisite: You have already installed QCL per [Installation](安装.md), and the server starts normally.
...
> ✅ Expected result: Platform status shows "Normal", and no health code is abnormal. If something is abnormal, note the health code and look it up in [Diagnostic Codes](../05-参考/诊断码.md).
...
> ❌ If you get "GUI not found: example", it means there is no GUI file with that id under `guis/` — you can build one yourself by following [Custom GUI](../02-服主指南/自定义GUI.md).
...
> 📖 For the full comparison table of all 10 sources, see [Item Source Reference](../02-服主指南/物品源引用.md).
...
> The root command `/qcl` has alias `/qinhcorelib`. For the full command and permission list, see [Commands and Permissions](../02-服主指南/命令与权限.md).

---

## 📚 Further Reading

- Next: [Core Concepts](核心概念.md) — Understand modules, bridges, item sources, diagnostics, and the API boundary
- [Configuration File](../02-服主指南/配置文件.md) — config.yml explained in detail
- [Diagnostics and Troubleshooting](../02-服主指南/诊断与排错.md) — What to do when status is abnormal
