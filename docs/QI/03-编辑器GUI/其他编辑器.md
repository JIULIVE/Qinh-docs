...
> 所属：[编辑器 GUI](概览.md)　·　相关：[碎片与模板](../02-服主指南/碎片与模板.md) · [资源包](../02-服主指南/资源包.md)
...
- **Lore / 碎片**（FragmentSelectionGui + LoreBlockGui）：
  - 分页管理引用的[碎片](../02-服主指南/碎片与模板.md) ID。
  - Lore 行增 / 改 / 删（聊天输入）。
- **变量**（VariableDeclarationGui）：
  - 分页管理[变量](../02-服主指南/变量.md)条目（键=值）。
  - 添加：输入键 → 输入值。
  - Shift + 右键编辑值；右键删除。

### 技能标签

显示动作表 + 触发器数 + 技能绑定概览，可跳到[动作 / 技能编辑器](动作编辑器.md)。
...
编辑物品的[自定义模型](../02-服主指南/资源包.md)。

- **CMD 编辑器**（CMDEditorGui）：设 `custom_model_data`。
- **模型选择器**（ModelSelectorGui）：从 `integrations/resource_models.yml` 的[模型目录](../02-服主指南/资源包.md#3-模型目录编辑器用)分类里选模型。

> 注意 QI 对资源包字段有[严格隔离校验](../02-服主指南/资源包.md#2-校验约束)，模型不能绑定变量 / 层 / 条件。

---

## 3. 段 / 词缀编译（SectionAffixCompileGui）

用于查看 / 编译[段](../02-服主指南/段.md)与[词缀](../02-服主指南/词缀.md)。

---

## 4. 内容包编辑（ItemPackEditorGui）

`/qi packs gui` 打开，管理 `.qipack` 内容包的导出 / 导入 / 同步。导入导出机制见 [导入导出](../04-开发者/导入导出.md)。
...
校验规则（你可能遇到的报错）见 [资源包 → 校验约束](../02-服主指南/资源包.md#2-校验约束)。
