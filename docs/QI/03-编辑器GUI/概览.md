...
> 所属：[编辑器 GUI](概览.md)　·　子页：[物品编辑器](物品编辑器.md) · [动作编辑器](动作编辑器.md) · [套装编辑器](套装编辑器.md) · [其他编辑器](其他编辑器.md)
...
- **动作载荷不透明**：QI 不解读处理器 payload，但若处理器注册了 [载荷 schema](../04-开发者/动作处理器开发.md#payload-schema)，GUI 会生成带类型字段的表单。
