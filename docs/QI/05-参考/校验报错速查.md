...
> 所属：[参考](命令.md)　·　相关：[物品定义](../02-服主指南/物品定义.md) · [资源包](../02-服主指南/资源包.md) · [层与装配](../04-开发者/层与装配.md)
...
| `unknown type=$type` | 类型不在 `item_types.yml` | 用合法类型，见 [物品类型](../02-服主指南/物品类型.md) |
...
全部为 **错误**。详见 [资源包 → 校验约束](../02-服主指南/资源包.md#2-校验约束)。
...
> 警告不阻止保存，但运行时该处理器会返回 `HANDLER_UNAVAILABLE`。处理器列表见 [处理器](../02-服主指南/动作系统/处理器.md)。
...
**允许的载体键**：`value` / `raw` / `data` / `payload`。详见 [Provider 与桥](../04-开发者/Provider与桥.md)。
...
`strict: true`（默认）为错误，`false` 为警告。三个域：INSTANCE / LAYER / RUNTIME，详见 [层与装配 → 写域策略](../04-开发者/层与装配.md#3-写域策略writedomainpolicy)。
...
- [物品定义](../02-服主指南/物品定义.md)：正确写法
- [资源包](../02-服主指南/资源包.md)：模型隔离规则
- [命令](命令.md)：`/qi diagnose` / `/qi problems`
