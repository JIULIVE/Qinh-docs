...
> 所属：[参考](命令.md)　·　相关：[配置文件（分段说明版）](配置文件.md)

本页贴出 `plugins/QinhItems/config.yml` 的**完整默认原文**，并在每段后逐行批注。想看按主题归类的说明版，看 [config.yml 全配置](配置文件.md)。
...
机制见 [层与装配 → 写域策略](../04-开发者/层与装配.md#3-写域策略writedomainpolicy)，相关报错见 [校验报错速查 §9](校验报错速查.md#9-写域违规writedomainpolicy)。
...
- `action-routing.strict: true` — 动作里出现逻辑字段（if/when/switch…）报错；`false` 仅警告。见 [动作系统 → 设计](../02-服主指南/动作系统/概览.md#3-关键设计yaml-里不能写逻辑)。
...
完整说明见 [宝石孔](../02-服主指南/宝石孔.md) 与 [集成](../04-开发者/集成.md)。
...
见 [属性与数值](../02-服主指南/属性与数值.md#3-配置属性映射)。

---

## 下一步

- [config.yml 全配置（分主题说明版）](配置文件.md)
- [校验报错速查](校验报错速查.md)
- [命令](命令.md)
