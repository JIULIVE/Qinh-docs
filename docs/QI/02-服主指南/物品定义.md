...
> 所属：[服主指南](物品定义.md)　·　相关：[物品类型](物品类型.md) · [属性与数值](属性与数值.md) · [动作系统](动作系统/概览.md)

这是 QI 最核心的一章：如何用 YAML 描述一件物品。读完你能看懂、写出任意复杂度的物品。

> 📦 想要现成配方直接抄？看 [物品示例库（~140 件内置示例）](物品示例库.md)。　🩺 报错了？看 [校验报错速查](../05-参考/校验报错速查.md)。
...
|---|---|---|---|
| （顶层键） | String | — | **物品 ID**（唯一） |
| **`type`** | String | 从文件名推断 / `misc` | **物品类型**，见 [物品类型](物品类型.md) |
...
| **`tier`** | String? | null | **品质**（大写，如 `EPIC`），见 [品质](品质与显示.md) |
| **`providers`** | Map | 空 | **数据提供者**（属性 `ap`、绑定效果等），见下文 §4 |
| **`variables`** | Map\<String,String\> | 空 | **变量**，见 [变量](变量.md) |
| `base_values` | Map\<String,Double\> | 空 | 基础值（编译进 `ap`），见 [属性](属性与数值.md) |
| `enchantments` | Map\<String,Int\> | 空 | 附魔：`键=等级` |
| `enchant_max_total_level` | Int? | null | 该物品的附魔总等级上限，见 [附魔上限](附魔上限.md) |
| `gem-sockets` | List\<String\> | 空 | 宝石孔类型 ID 列表，见 [宝石孔](宝石孔.md) |
| `gem-state` | Map | 空 | 各孔当前镶嵌状态 |
| `gem-lore-override` | List\<String\> | 空 | 自定义宝石孔 Lore（覆盖默认） |
| `sections` | List\<String\> | 空 | 引用的[段](段.md) ID |
| `affixes` | List\<String\> | 空 | 引用的[词缀](词缀.md) ID |
| `fragments` | List\<String\> | 空 | 引用的[碎片](碎片与模板.md) ID |
| `resource_pack` | 段 | null | [资源包 / 自定义模型](资源包.md) |
| **`options`** | 段 | — | **物品选项**（见下文 §3） |
| `actions` | 段 | — | **动作 / 触发器**（见 [动作系统](动作系统/概览.md)） |
...
写在 `options.restrictions` 列表里，装备 / 使用时校验，不满足会在 Lore 上标红并阻止使用（通过 [`canUse`](../04-开发者/API参考.md) / `QinhItemUseCheckEvent`）：
...
详见 [API 参考 → canUse](../04-开发者/API参考.md#使用条件检查)。
...
- **`ap`** — 战斗属性。详见 [属性与数值](属性与数值.md)。
- **`perm_effects`** — 穿戴时常驻的药水效果：`{"效果名":放大等级}`，0=I 级。
- **`legendinlay` / `magicgem`** — 宝石孔载荷，通常由 GUI / `gem-sockets` 自动同步生成，详见 [宝石孔](宝石孔.md)。

> Provider 也可以写成多载体键的子段形式，开发者细节见 [Provider 与桥](../04-开发者/Provider与桥.md)。允许的载体键：`value` / `raw` / `data` / `payload`。
...
- **接外部源时通常不用再配 `resource_pack.custom_model_data`**——外部物品本身已带模型。CMD / 资源包路线见 [资源包与自定义模型](资源包.md)。
...
> 这些前缀由 QinhCoreLib 统一提供，QI / QinhSkills 等全生态通用。完整说明见 [跨模块对接 → QI ↔ CoreLib](../04-开发者/跨模块对接.md#二qi--corelib)。
...
校验规则（保存时你可能遇到的报错）详见 [资源包](资源包.md#校验约束) 与开发者向的约束系统说明。

---

## 下一步

- 看有哪些类型可用、各能加什么 → [物品类型](物品类型.md)
- 给物品上属性 → [属性与数值](属性与数值.md)
- 让物品有主动技能 → [动作系统](动作系统/概览.md)
