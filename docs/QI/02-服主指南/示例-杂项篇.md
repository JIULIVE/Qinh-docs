...
> 所属：[服主指南](物品定义.md) → [物品示例库](物品示例库.md) · 杂项篇　·　相关：[物品类型](物品类型.md) · [动作系统](动作系统/概览.md)
...
经济货币类。底物多为原版贵金属材质，可堆叠；交互型货币以 [触发器](动作系统/触发器.md) `left_click` 配合 `consume` 扣自身完成兑换。
...
- 触发器 [触发器](动作系统/触发器.md) `left_click` + `cooldown: 3s` + `consume: self:1`：每次扣 1 张并 3 秒冷却。处理器 [处理器](动作系统/处理器.md) 链依次为 `qi:command`（向经济插件充值，`{player}` 替换为玩家名）、`qi:action_bar` 提示、`qi:sound` 反馈。

---

## 代币 token.yml

兑换券类。一律以 [触发器](动作系统/触发器.md) `left_click` 触发使用，配 `consume: self:1` 扣券；兑换结果可以是指令奖励，也可以是 `qi:give_item` 直接发放物品。
...
- [触发器](动作系统/触发器.md) `left_click` + `consume: self:1`；[处理器](动作系统/处理器.md) `qi:command` 调用礼包指令，再以音效与 action_bar 反馈。
...
- 处理器 [处理器](动作系统/处理器.md) `qi:give_item` 的 payload 是另一件 QI 物品 ID（`demo_void_reaver`，装备篇中的武器），扣券后直接入背包；再以 `qi:title` 全屏标题与音效强化兑换仪式感。

---

## 奖杯 trophy.yml

纪念 / 战利品类。多数为纯外观陈列品（`material` + `display_name` + `glow`）；如需可交互纪念品，可挂 [触发器](动作系统/触发器.md) `left_click`。
...
- 无 `consume`（炫耀不损耗）；[处理器](动作系统/处理器.md) `qi:console_command` 以控制台权限执行 `broadcast` 向全服播报，`{player}` 替换为持有者名。

---

## 头颅 skull.yml

装饰头颅类。多为纯外观；玩家头通过 `options.skull_owner` 指定皮肤主人。可挂 `left_click` / `shift_left_click` 等 [触发器](动作系统/触发器.md)。
...
- **skull_owner 机制**：`player_head` 渲染为 `Herobrine` 皮肤头。触发器 [触发器](动作系统/触发器.md) `shift_left_click`（潜行 + 左键）+ `consume: self:1` 实现"献祭"；[处理器](动作系统/处理器.md) `qi:give_item` 发放上文的 `demo_creeper_trophy`，形成头颅互换链。

---

## 道具 prop.yml

通用道具类。靠 [触发器](动作系统/触发器.md) `left_click` 当"使用"手势触发 `qi:title` / `qi:sound` / `qi:command` 等效果。
...
- `cooldown: 2m`（分钟级冷却）+ `consume: self:1`（一次性）；[处理器](动作系统/处理器.md) `qi:command` 以玩家身份执行 `spawn` 回到出生点。
...
坐骑相关类。外观件为鞍具 / 缰绳；召唤型用刷怪蛋底物 + [触发器](动作系统/触发器.md) 触发 `qi:console_command` 的 `summon` 指令生成坐骑实体。
...
- **召唤机制**：QI 本身不内置生物生成，而是靠 [处理器](动作系统/处理器.md) `qi:console_command` 执行原版 `summon minecraft:horse ~ ~ ~ {Tame:1b}`——`~ ~ ~` 为玩家所在坐标，NBT `{Tame:1b}` 让马生成即已驯服。`consume: self:1` 使召唤蛋一次性。
...
- **召唤机制**：[处理器](动作系统/处理器.md) `qi:console_command` 召唤 `minecraft:skeleton_horse`（骷髅马，同样 `{Tame:1b}` 驯服）。`options.bind_on_acquire: true` 获得即绑定，`restrictions: level:30` 限制 30 级以下不可使用；触发器 [触发器](动作系统/触发器.md) 为 `shift_left_click`（潜行 + 左键）。此例不带 `consume`，可重复召唤（受 10 秒冷却约束）。
