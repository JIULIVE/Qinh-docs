...
> 所属：[服主指南](物品定义.md)　·　相关：[属性与数值](属性与数值.md) · [动作系统](动作系统/概览.md)
...
- 套装属性走 [属性系统](属性与数值.md) 的独立源；套装 perm 效果走 `PermEffectSync`。
...
技能 `refs` 里的 `handler` + `payload` 由 QI 派发（如 `qinhskills:cast` 交给 [QinhSkills](../04-开发者/集成.md)）。冷却 `cooldown` 支持 `5s` / `200ms` / `3t`（tick）。
...
GUI 编辑套装见 [套装编辑器](../03-编辑器GUI/套装编辑器.md)。

---

## 下一步

- [动作系统](动作系统/概览.md)：套装技能用的处理器
- [属性与数值](属性与数值.md)：套装属性如何上身
- [套装编辑器](../03-编辑器GUI/套装编辑器.md)：GUI 编排套装
