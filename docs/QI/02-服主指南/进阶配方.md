...
> 所属：[服主指南](物品定义.md)　·　相关：[变量](变量.md) · [段](段.md) · [套装](套装.md)
...
> 变量只影响**显示**；真正的属性数值在 `providers.ap`。要让显示与属性一致，需自己保持两者协调，或用[随机生成](随机生成.md)的容量 / 词缀机制。变量机制见 [变量](变量.md)。
...
渲染顺序：品质前缀 + 词缀前缀 → 物品名 → 词缀后缀 + 守护后缀，Lore 段累积每个段 / 词缀的描述行。段机制见 [段](段.md)，词缀见 [词缀](词缀.md)，显示顺序见 [品质与显示](品质与显示.md#5-lore-段顺序从上到下)。
...
物品引用 `quality_damage_pool`，随机生成时按物品 tier 选对应子池抽取。随机生成见 [随机生成](随机生成.md#5-词缀池抽取)。
...
事件型 abilities 穿满件数后**自动触发**（与手持物品无关）。套装机制见 [套装](套装.md)，技能交给 [QinhSkills](../04-开发者/集成.md)。
...
强化系统可通过 [层 API / 变量 API](../04-开发者/API配方集.md#配方-4强化--镶嵌系统写层补丁) 在运行时改 `star`。
...
绑定见 [灵魂绑定](灵魂绑定.md)，限制见 [物品定义](物品定义.md#restrictions-限制语法)，动作见 [动作系统](动作系统/概览.md)。

---

## 下一步

- [变量](变量.md) / [段](段.md) / [套装](套装.md)：底层机制
- [随机生成](随机生成.md)：词缀池 + 容量
- [物品示例库](物品示例库.md)：内置示例
