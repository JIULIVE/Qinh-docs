...
> 所属：[服主指南](物品定义.md)　·　相关：[变量](变量.md) · [层与装配](../04-开发者/层与装配.md)
...
碎片 / 物品可声明自定义模型，详见 [资源包](资源包.md)。`resource_pack` 段最终编译成 `ResourcePackDeclaration`。
...
未显式配置时会从 `QinhItemTypeRegistry` 按 `library-category` 自动生成。开发者细节见 [层与装配](../04-开发者/层与装配.md)。
