...
> 所属：[服主指南](../物品定义.md) → 动作系统　·　子页：[触发器](触发器.md) · [处理器](处理器.md) · [冷却/消耗/条件](冷却消耗条件.md)
...
**需要复杂逻辑怎么办？** 由开发者实现一个 [动作处理器](../../04-开发者/动作处理器开发.md)，把 if/switch/状态判断写进 Kotlin/Java，YAML 只负责引用它并传 payload。
...
- **触发器（Trigger）**：100+ 原子。点击类（left/right/shift）、潜行（单击 / 双击 / 序列）、移动（冲刺 / 滑翔 / 跳）、战斗（命中 / 受击 / 击杀 / 暴击）、容器 / 合成、实体交互、环境、服务器事件、`tick` 周期。完整清单见 [触发器](触发器.md)。
- **处理器（Handler）**：内置 10 个（`qi:message` / `qi:action_bar` / `qi:title` / `qi:subtitle` / `qi:sound` / `qi:bossbar` / `qi:command` / `qi:console_command` / `qi:give_item` / `qi:close_inventory`）+ 生态（`qinhskills:cast` / `combat:swing`）。每个的 payload 格式见 [处理器](处理器.md)。
...
- [触发器原子大全](触发器.md) —— 100+ 触发原子逐个列出
- [动作处理器大全](处理器.md) —— 每个 handler 的 payload 格式
- [冷却 / 消耗 / 条件 / 连招](冷却消耗条件.md) —— 语法细节
- 开发自定义处理器 → [动作处理器开发](../../04-开发者/动作处理器开发.md)
