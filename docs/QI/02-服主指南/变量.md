...
> 所属：[服主指南](物品定义.md)　·　相关：[品质与显示](品质与显示.md) · [碎片与模板](碎片与模板.md)
...
在物品（或[碎片](碎片与模板.md)）的 `variables:` 段声明，键是变量名，值是**范围**或**静态字符串**：
...
| `star` | 星级，渲染名尾 `[+N]`（部分类型） |
| `soulbound.owner` | 灵魂绑定时设置，见 [灵魂绑定](灵魂绑定.md) |
...
通过门面 `QinhItemsAPI.variables()`（详见 [API 参考](../04-开发者/API参考.md)）：
...
- [词缀](词缀.md) / [段](段.md)：用池化文本做前后缀
- [碎片与模板](碎片与模板.md)：复用变量声明
- [API 参考](../04-开发者/API参考.md)：variables 门面完整签名
