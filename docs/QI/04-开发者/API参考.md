...
> 所属：[开发者](API概览.md)　·　相关：[API 概览](API概览.md) · [事件](事件.md)
...
层机制见 [层与装配](层与装配.md)。`LayerWriteResult`：`OK` / `NOT_QINH_ITEM` / `DOMAIN_VIOLATION` / `PROVIDER_PATCH_FORBIDDEN`。
...
`InstanceWriteResult`：`OK` / `NOT_QINH_ITEM` / `LOCKED_BY_OTHER` / `DOMAIN_VIOLATION`。详见 [变量](../02-服主指南/变量.md#7-开发者-api)。
...
详见 [属性与数值](../02-服主指南/属性与数值.md#8-开发者读属性)。
...
物品模板。字段全表见 [物品定义](../02-服主指南/物品定义.md#2-顶层字段总表)。
