...
> 所属：[开发者](API概览.md)　·　相关：[事件大全](事件.md) · [API 配方集](API配方集.md)

[事件大全](事件.md) 列签名，本页给每个事件一段**完整可用的 listener**（Java），含典型用例。
...
可取消（`setCancelled`）。也可通过 `QinhBridgeHook` 接入，见 [Provider 与桥](Provider与桥.md)。
