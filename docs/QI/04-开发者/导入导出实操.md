...
> 所属：[开发者](API概览.md)　·　相关：[导入导出（API）](导入导出.md) · [命令](../05-参考/命令.md)

[导入导出](导入导出.md) 讲 API 与格式，本页讲**从 MMOItems / NeigeItems 迁移到 QI 的完整步骤**、字段映射对照、dry-run 解读。
...
> 核心三项进 base_values（受 base 编辑器管理），其余进 `providers.ap`。比率类自动 ÷100。最终都交 AttributePlus，记得配好 [attribute-mapping](../02-服主指南/属性与数值.md#3-配置属性映射)。
...
> `level` / `permission` / `world` 由 QI 原生判；`class` / 阵营等委托给 [`QinhItemUseCheckEvent`](事件.md)，需你实现职业判断。
...
- [事件](事件.md)：QinhItemUseCheckEvent
