...
> 所属：[开发者](API概览.md)　·　相关：[Provider 与桥](Provider与桥.md) · [宝石孔](../02-服主指南/宝石孔.md)
...
物品动作里的 `qinhskills:cast` 处理器把技能释放交给 [QinhSkills](../02-服主指南/动作系统/处理器.md#生态处理器其它插件注册)（QS）。
...
孔目录 `integrations/legendinlay_sockets.yml` 的 `lore` 必须与 LI 那边逐字一致（LI 靠 Lore 识别孔），见 [宝石孔](../02-服主指南/宝石孔.md#4-孔-lore-渲染优先级)。
...
> 注意：**前缀解析（`qi:` / `qinhitems:`）在 CoreLib**，物品反查（item → id）在 QI。详见 [API 概览 → 接入路线](API概览.md#6-接入路线选择)。

---

## 下一步

- [Provider 与桥](Provider与桥.md)：桥抽象细节
- [导入导出](导入导出.md)
