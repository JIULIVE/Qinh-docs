...
> 所属：[开发者](API概览.md)　·　子页：[API 参考](API参考.md) · [事件](事件.md) · [动作处理器开发](动作处理器开发.md)

本章给插件开发者：如何依赖 QI、入口在哪、API 怎么分层。完整方法签名见 [API 参考](API参考.md)。
...
| 你想… | 用 | 章节 |
|---|---|---|
| 按 ID 造物品、反查物品归属 | QI `assembly()` / `isQinhItem` | [API 参考](API参考.md) |
| 用 `qi:id` / `qinhitems:id` 前缀解析物品 | CoreLib `ItemManagerAPI.getHookItem(ref)` | [集成](集成.md) |
| 给物品挂外部系统数据 | Provider + Bridge | [Provider 与桥](Provider与桥.md) |
| 自定义触发后的逻辑 | 动作处理器 | [动作处理器开发](动作处理器开发.md) |
| 监听物品生命周期 | 事件 | [事件](事件.md) |
| 打孔 / 镶嵌 / 强化（改物品后状态） | 层 API | [层与装配](层与装配.md) |
| 市场 / 邮件 / 抽奖 / 强化等成品代码 | — | [API 配方集](API配方集.md) |
| 接 AP / QinhSkills / 宝石后端 | — | [集成实操](集成实操.md) |
