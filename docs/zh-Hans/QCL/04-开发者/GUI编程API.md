> 相关：[API概览.md](./api-overview.md) · [数据存储与占位符.md](./data-placeholders.md) · [../02-服主指南/自定义GUI.md](../02-server-guide/custom-gui.md) · [../05-参考/术语表.md](../05-reference/glossary.md)

# 🖼️ GUI 编程 API（CustomGuiManager）

本页讲**编程式**操作 GUI —— 用 Kotlin 代码打开 / 渲染 / 填数据。GUI 的 **YAML 字段**（布局、图标、点击动作等）请看服主文档 [../02-服主指南/自定义GUI.md](../02-server-guide/custom-gui.md)，本页只讲代码这一面。
...
- [../02-服主指南/自定义GUI.md](../02-server-guide/custom-gui.md) —— GUI 的 YAML 字段与服主配置
- [数据存储与占位符.md](./data-placeholders.md) —— PapiBridge 与 %qcl_xxx% 占位符
- [动作与技能桥.md](./actions-skill-bridge.md) —— 点击动作背后的动作系统
