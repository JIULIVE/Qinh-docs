...
下面是 QCL 1.3.0 默认生成的完整内容，可直接作为照抄模板：

```yaml
compatibility:
  # 严格版本检查。false（默认，推荐）：服务端版本高于「已测试版本」时仅打印警告并照常启动；
  # true：高于已测试版本直接拒绝启动。低于最低支持版本（1.21.11）始终拒绝，与此开关无关。
...
QCL 是整套生态的硬依赖，一旦它因版本检查拒绝启动，QI/QS/QC/QR 等都会跟着无法加载。这里控制「服务端版本高于 QCL 已测试版本」时的行为。

| 键 | 默认值 | 可选值 | 作用 |
|---|---|---|---|
| `compatibility.strict-version-check` | `false` | `false` / `true` | `false`（推荐）=向上兼容：服务端版本高于「已测试版本」时仅打印一条警告并照常启动，不会因为出了新版本就拒绝运行（避免「升级新版后用不了」）；`true`=严格模式：高于已测试版本直接拒绝启动，适合宁可不启动也不冒险的服主 |

> ⚠️ 低于**最低支持版本（Minecraft 1.21.11）**始终拒绝启动，与此开关无关。当前已测试到 **1.21.x**，标注兼容 **1.21.11 ~ 26.2.x**。改完 `/qcl reload` 不重读此项，需**重启服务器**。
