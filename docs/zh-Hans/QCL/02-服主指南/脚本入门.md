...
> 相关：[自定义GUI.md](./custom-gui.md) · [配置文件.md](./config.md) · [物品源引用.md](./item-source-references.md) · [开发者 / 脚本API.md](../04-developer/script-api.md)
...
> 💰 经济方法的 `provider` / `currency` 是可选参数，不填用默认经济。物品的 `ref` 就是**统一物品源引用**（见 [物品源引用.md](./item-source-references.md)）。
...
> 想做沙箱外的事（比如读外部文件）？那已经超出服主脚本范畴，应该改用子插件或专门的开发能力，见 [脚本API.md](../04-developer/script-api.md)。
...
- [自定义GUI.md](./custom-gui.md) —— 把脚本挂到 GUI 的条件与动作上
- [物品源引用.md](./item-source-references.md) —— `qcl.itemGive` / `itemParse` 用的引用格式
- [配置文件.md](./config.md) —— `javascript.*` 三项配置的完整说明
- [开发者 / 脚本API.md](../04-developer/script-api.md) —— 更深入的脚本能力与 API 细节
