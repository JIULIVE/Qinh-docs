...
装了 PlaceholderAPI 后，**任意插件**（计分板、全息、MythicMobs 技能、聊天）都能直接读 QCL 属性，无需依赖 QS/QC。前缀统一 `qinhcorelib`：

| 占位符 | 返回 |
|---|---|
| `%qinhcorelib_combatpower%` | 玩家战斗力 |
| `%qinhcorelib_attr_<key>%` | 某属性的合计总值（如 `%qinhcorelib_attr_physical_damage%`） |
| `%qinhcorelib_attrpct_<key>%` | 同上但按百分比显示（如 `20%`） |
| `%qinhcorelib_attrname_<key>%` | 该属性的显示名 |
| `%qinhcorelib_cdr%` | 冷却缩减小数（如 `0.3`，上限 `0.9`） |
| `%qinhcorelib_cdrmult%` | `1 - 冷却缩减`，即「乘到基础冷却上的系数」 |
| `%qinhcorelib_cdrapply_<基础冷却>%` | 把基础冷却套上缩减后的**最终冷却**，如 `%qinhcorelib_cdrapply_20%` |
| `%qinhcorelib_reduceapply_<属性>_<基础值>%` | 通用「按某个 0~1 属性削减基础值」，如 `%qinhcorelib_reduceapply_damage_reduction_100%` |

> 💡 **为什么重要**：冷却缩减不止 QS 能用。不装 QS、只用 MythicMobs + QCL 的服，可在 MM 技能的冷却字段里写 `%qinhcorelib_cdrapply_<技能基础冷却>%`，让玩家的冷却缩减属性照样生效——把「属性 → 实际效果」的换算放在 CoreLib 层，所有前端通吃。
