...
> 所属：[开发者](./api-overview.md)　·　相关：[API 参考](./api-reference.md) · [事件](./events.md) · [集成实操](./integration-howto.md)
...
> 若想用 QI 自带的随机品质 / 词缀生成，见 [随机生成](../02-server-guide/random-generation.md) 的 `RandomItemGenerator`。
...
写入结果见 [变量](../02-server-guide/variables.md#7-开发者-api)。
...
物品里这样配：`options.restrictions: ["class:战士"]`。事件全表见 [事件](./events.md)。
...
YAML 里引用：`- handler: myplugin:teleport_home / payload: "..."`。详见 [动作处理器开发](./handler-development.md)。
...
- [API 参考](./api-reference.md)：全部方法签名
- [事件](./events.md)：可监听的生命周期
- [集成实操](./integration-howto.md)：接外部插件
