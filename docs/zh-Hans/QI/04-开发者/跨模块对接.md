...
> 所属：[开发者](./api-overview.md)　·　相关：[集成](./integration.md) · [集成实操](./integration-howto.md) · [属性与数值](../02-server-guide/attributes-numbers.md)
...
诊断：`ItemManagerAPI.diagnose(ref)` 返回 `DiagnosticResult`，码含 `SOURCE_NOT_FOUND`（前缀没注册=插件没装）、`ITEM_NOT_FOUND`（源里没这 ID）、`ITEM_REF_PARSE_FAILED`（格式错）。服主侧见 [物品定义 §5.1](../02-server-guide/item-definition.md#51-外部物品源引用craftengine--itemsadder--nexo-等)。
...
所以：要 `qi:` 前缀解析依赖 CoreLib；要反查 / 直接生成只依赖 QI。接入路线见 [API 概览](./api-overview.md#6-接入路线选择)。
...
链接机制见 [集成 → QinhSkills](./integration.md#2-qinhskills-集成)，接入步骤见 [集成实操](./integration-howto.md#二qinhskills技能引擎)。
...
见 [宝石孔](../02-server-guide/gem-sockets.md) 与 [集成 → Legendinlay](./integration.md#3-legendinlay-集成宝石孔后端-a)。
...
启动顺序细节见 [配置文件 → 启动顺序](../05-reference/config.md#启动顺序参考)。

---

## 下一步

- [集成（架构）](./integration.md)：注册表 / 桥
- [集成实操](./integration-howto.md)：逐插件接入
