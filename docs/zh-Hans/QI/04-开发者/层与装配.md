...
> 所属：[开发者](./api-overview.md)　·　相关：[核心概念](../01-getting-started/core-concepts.md) · [API 参考](./api-reference.md)
...
> 要改属性，走 [Provider](./providers-bridges.md) 而非层补丁。
...
配置见 [config.yml → write-domains](../05-reference/config.md#写域隔离-write-domains)。越权写入在 `strict: true` 时报错。
...
`TemplateCompiler` 把 YAML 编译成 `QinhItemDefinition`：合并碎片 → override → 根字段，注入 tier 变量，迁移旧 effects，编译基础值进 ap，读 actionLore / 宝石孔 / 段 / 词缀 / 附魔 / 资源包。合并规则见 [碎片与模板](../02-server-guide/fragments-templates.md#4-合并规则模板编译)。
...
服主向说明见 [碎片与模板 → 库清单](../02-server-guide/fragments-templates.md#8-库清单library-manifest)。
