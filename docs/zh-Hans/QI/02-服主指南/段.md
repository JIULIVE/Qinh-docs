...
随机生成时，`quality_pool` 会进一步把命中的子段当 `weight_join` 池抽取，详见 [随机生成](./random-generation.md)。
...
2. 捕获第一个前缀 / 后缀。
3. 累积所有 Lore 行。
4. 再处理 `affixes:` 引用的[词缀](./affixes.md)。

返回 `RenderResult(prefix, suffix, loreLines, warnings)`，并入[结构化 Lore](./quality-display.md#5-lore-段顺序从上到下) 的「段 Lore」段。
...
管理命令：`/qi sections`、`/qi sections summary`、`/qi sections formats`、`/qi sections format`，见 [命令](../05-reference/commands.md)。
...
- [随机生成](./random-generation.md)：把段 / 词缀池接进随机掉落
- [词缀](./affixes.md)：定义具名词缀
