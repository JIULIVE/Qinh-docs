...
> 所属：[服主指南](./item-definition.md) → [物品示例库](./item-cookbook.md) · 饰品消耗篇　·　相关：[物品类型](./item-types.md) · [宝石孔](./gem-sockets.md) · [动作系统](./action-system/overview.md)
...
- 无 `actions`，属性由 `providers.ap` 被动结算，不涉及触发；属性 JSON 写法见 [物品定义](./item-definition.md)。
...
- `equip` [触发器](./action-system/triggers.md) 是饰品被动反馈入口；`qi:action_bar` + `qi:sound` 两个 [处理器](./action-system/handlers.md)。
...
- `equip` / `unequip` 两个 [触发器](./action-system/triggers.md) 各配反馈；`options.restrictions: ["level:15"]` 是使用限制，见 [冷却/消耗/条件](./action-system/cooldown-cost-conditions.md)。
...
- `equip` [触发器](./action-system/triggers.md) + 紫水晶 chime 音效。
...
- `options.bind_on_acquire: true` 获得即绑定；`equip` / `unequip` 双 [触发器](./action-system/triggers.md)。
...
- 底物用 `nether_star`；`equip` [触发器](./action-system/triggers.md) 配 `qi:title` [处理器](./action-system/handlers.md)。
...
- 同时具备 `bind_on_acquire` 与 `restrictions: ["level:20"]`，限制语义见 [冷却/消耗/条件](./action-system/cooldown-cost-conditions.md)。
...
- 三维属性（攻击/防御/生命）+ `equip` [触发器](./action-system/triggers.md)。
...
- `bind_on_acquire` + `equip` / `unequip` 双 [触发器](./action-system/triggers.md)。
...
食物专属字段 `options.food.nutrition` / `saturation` 自定义饱食恢复，详见 [物品定义](./item-definition.md)。
...
- `consume` [触发器](./action-system/triggers.md) 仅可食用材质生效；`options.food` 自定义 `nutrition: 8` / `saturation: 12.8`。
...
- `cooldown: 30s` 防刷增益；`qi:console_command` 以控制台权限发 `effect give {player} ...`，冷却语义见 [冷却/消耗/条件](./action-system/cooldown-cost-conditions.md)。
...
- `consume: ["self:1"]` 一次性消耗 + `cooldown: 3s`，见 [冷却/消耗/条件](./action-system/cooldown-cost-conditions.md)；`qi:command` 以玩家身份执行 `spawn`。
...
- 与回城卷轴同结构，`cooldown: 5s` + `self:1`，三个反馈 [处理器](./action-system/handlers.md)。
...
- `place_block` [触发器](./action-system/triggers.md) 在种子撒入耕地时触发；`cooldown: 2s`。
...
- `left_click` [触发器](./action-system/triggers.md) + `cooldown: 3s`，无消耗。
...
- `consume: ["self:3"]` 一次吃掉 3 个本物品，`qi:give_item` [处理器](./action-system/handlers.md) 产出 `demo_iron_fragment`；消耗语义见 [冷却/消耗/条件](./action-system/cooldown-cost-conditions.md)。
...
- `break_block` [触发器](./action-system/triggers.md) + `cooldown: 2s`。
...
- `break_block`（`cooldown: 1s`）与 `left_click`（`cooldown: 500ms`）两个独立 [触发器](./action-system/triggers.md)，各设不同冷却；`restrictions: ["level:10"]`。
...
- `fish` [触发器](./action-system/triggers.md) + `cooldown: 2s`。
...
- `fish` 触发 + `cooldown: 5s`；`qi:give_item` [处理器](./action-system/handlers.md) 赠出 `demo_angler_rod`。
...
- [示例-装备篇](./cookbook-equipment.md) —— 武器、护甲及其 [宝石孔](./gem-sockets.md) 与战斗触发示例
- [示例-杂项篇](./cookbook-misc.md) —— 其余物品类型与进阶动作组合
- [物品示例库总览](./item-cookbook.md) —— 返回示例库索引页
