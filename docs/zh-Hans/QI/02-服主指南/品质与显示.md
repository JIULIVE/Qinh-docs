...
> 所属：[服主指南](./item-definition.md)　·　相关：[词缀](./affixes.md) · [随机生成](./random-generation.md)
...
> **容量怎么算成槽位数？** 见 [随机生成 → 容量计算](./random-generation.md#3-容量计算)。公式：`base + scale×rand + spread×rand + maxSpread×rand`，向下取整、不小于 0。
...
| 5 | **段 Lore** | [Section](./sections.md) 渲染 | 词缀池产出的行 |
| 6 | **宝石状态** | `gem-state` | `▪ 已镶嵌 xxx` |
| 7 | **动作 Lore** | 技能描述 | `▸ 雷霆斩` |
| 8 | **宝石空孔** | `gem-sockets` | `[ + ] 空孔` |
| 9 | **套装 Lore** | [Set](./sets.md) 渲染 | 套装件数 / 加成 |
| 10 | **描述 / 风味** | `lore:` 字段 | 你写的描述行 |
| 11 | **需求** | `options.restrictions` | `✔ 需求: 等级 20` / `✘ 需求: …` |
| 12 | **灵魂绑定** | 实例数据 | `xxx 的物品`，见 [灵魂绑定](./soulbinding.md) |
...
1. 先做变量替换（`{变量}`）。
2. 再叠加[段](./sections.md) / [词缀](./affixes.md)产出的**前缀 / 后缀**。
...
- 让品质参与随机掉落 → [随机生成](./random-generation.md)
- 用词缀池给名字加前后缀 → [词缀](./affixes.md) / [段](./sections.md)
- 给属性配数值 → [属性与数值](./attributes-numbers.md)
