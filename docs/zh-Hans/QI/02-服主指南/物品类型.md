...
> 所属：[服主指南](./item-definition.md)　·　相关：[物品定义](./item-definition.md) · [品质与显示](./quality-display.md)

物品类型决定一件物品「**属于哪一类**」以及「**能用哪些功能**」。配置文件：`plugins/QinhItems/item_types.yml`。

> 📋 想看每个类型的**默认材质 / 编辑器图标 / 库分类**全表？看 [资源文件总览 → item_types 完整表](../05-reference/resource-files.md#五item_typesyml-完整类型表)。
...
注意：类型必须存在，否则物品会因「类型未知」而 unhealthy（见 [物品定义 → 健康检查](./item-definition.md#7-物品健康检查)）。

---

## 6. 类型与「库（Library）」

类型可以归入一个**库分类（library-category）**。库（`LibraryManifest`）是把一组类型 / 能力打包成可分发内容包的元数据描述，主要给内容分发 / 编辑器分组用。开发者细节见 [层与装配 → 库清单](../04-developer/layers-assembly.md)。

---

## 下一步

- [品质与显示](./quality-display.md)：给类型加品质、控制 Lore 排版
- [属性与数值](./attributes-numbers.md)：给 ATTRIBUTE 类型加战斗属性
- [宝石孔](./gem-sockets.md)：给 GEM_SOCKET 类型打孔
