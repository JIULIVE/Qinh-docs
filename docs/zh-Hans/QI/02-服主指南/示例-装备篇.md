...
> 所属：[服主指南](./item-definition.md) → [物品示例库](./item-cookbook.md) · 装备篇　·　相关：[物品类型](./item-types.md) · [属性与数值](./attributes-numbers.md) · [动作系统](./action-system/overview.md)

本页逐件讲解 QI 内置的装备类示例物品。每个文件的文件名（如 `armor`）即物品 `type`，每个顶层键即一件物品 ID。改完任意 yml 后需执行 `/qi reload` 才生效——动作只在 reload / 重启时加载。

数值由 `providers.ap.value`（一段属性 JSON）交 [AttributePlus](./attributes-numbers.md) 应用，QI 本身不内置数值。详见 [属性与数值](./attributes-numbers.md)。
...
- 入门基线：`providers.ap.value` 提供 `defense` / `health`（[属性与数值](./attributes-numbers.md)），`unbreakable` 不掉耐久。无 `tier` 时默认 COMMON（[品质与显示](./quality-display.md)）。
...
- `tier: EPIC` 配 `glow: true` 形成史诗外观（[品质与显示](./quality-display.md)）。
...
- 首个带动作的护甲：`equip` [触发器](./action-system/triggers.md) 驱动 `qi:action_bar` + `qi:sound` [处理器](./action-system/handlers.md)，`payload` 中音效格式为 `声音键;音量;音调`。
...
- 入门基线，仅 `defense:4`（[属性与数值](./attributes-numbers.md)）。
...
- `tier: RARE` + `glow`（[品质与显示](./quality-display.md)）。
...
- `equip` [触发器](./action-system/triggers.md) → `qi:action_bar` + `qi:sound`（下界合金装备音效）[处理器](./action-system/handlers.md)。
...
- 演示一物多触发：`equip` 带 `cooldown: 2s`，用 `qi:title`（格式 `标题||副标题||淡入||停留||淡出`）；`unequip` 用 `qi:message`（[触发器](./action-system/triggers.md) · [处理器](./action-system/handlers.md)）。
...
- 入门基线（[属性与数值](./attributes-numbers.md)）。
...
- `tier: EPIC` + `glow`（[品质与显示](./quality-display.md)）。
...
- `equip` [触发器](./action-system/triggers.md) → `qi:title` + 信标激活音效 [处理器](./action-system/handlers.md)。
...
- `bind_on_acquire: true` 获得即 [灵魂绑定](./soulbinding.md)；`restrictions: level:20` 限制 20 级使用。`equip` 用 `qi:bossbar`（格式 `文本;进度;颜色;样式;时长tick`），`unequip` 提示卸下（[触发器](./action-system/triggers.md) · [处理器](./action-system/handlers.md)）。
...
- `equip` [触发器](./action-system/triggers.md) → `qi:title` + 装备音效 [处理器](./action-system/handlers.md)。
...
- `bind_on_acquire` [灵魂绑定](./soulbinding.md) + `restrictions: level:20`，`equip` / `unequip` 双 [触发器](./action-system/triggers.md) 各发 `qi:action_bar`（[处理器](./action-system/handlers.md)）。
...
- 引入 `movement_speed` 移速属性（[属性与数值](./attributes-numbers.md)），`tier: RARE` + `glow`。
...
- `equip` / `unequip` 双 [触发器](./action-system/triggers.md) 各发 `qi:action_bar`，equip 附音效（[处理器](./action-system/handlers.md)）。
...
- 最高移速 `0.12`，`bind_on_acquire` [灵魂绑定](./soulbinding.md) + `restrictions: level:20`，`equip` [触发器](./action-system/triggers.md) 发 `qi:title` + 龙翼振翅音效（[处理器](./action-system/handlers.md)）。
...
- 入门基线，`attack_damage:6`（[属性与数值](./attributes-numbers.md)）。
...
- `left_click` [触发器](./action-system/triggers.md)（最可靠的手势）+ `cooldown: 2s`，`crit_rate` 暴击率属性（[属性与数值](./attributes-numbers.md)）。
...
- `on_hit` [触发器](./action-system/triggers.md)（箭矢命中实体时），`bind_on_acquire` [灵魂绑定](./soulbinding.md) + `restrictions: level:20`。
...
- `left_click` [触发器](./action-system/triggers.md) + `cooldown: 2s`，含 `crit_rate`（[属性与数值](./attributes-numbers.md)）。
...
- `on_hit` [触发器](./action-system/triggers.md)，`bind_on_acquire` [灵魂绑定](./soulbinding.md) + `restrictions: level:20`。
...
- `left_click` [触发器](./action-system/triggers.md) + `cooldown: 3s`，`qi:title` + 激流音效 [处理器](./action-system/handlers.md)。
...
- `material: spectral_arrow` + `glow`（[品质与显示](./quality-display.md)）。
...
- `on_hit` [触发器](./action-system/triggers.md) + `cooldown: 2s`，无 `providers` 仅靠动作表现（[处理器](./action-system/handlers.md)）。
...
- 唯一带 `providers` 的箭（`attack_damage:6`，[属性与数值](./attributes-numbers.md)），`restrictions: level:15`，`on_hit` 发 `qi:title`（[触发器](./action-system/triggers.md) · [处理器](./action-system/handlers.md)）。
...
- `equip` [触发器](./action-system/triggers.md) → `qi:action_bar` + 盾牌格挡音效 [处理器](./action-system/handlers.md)。
...
- 演示 `on_damage` 受击 [触发器](./action-system/triggers.md) + `cooldown: 2s`，`bind_on_acquire` [灵魂绑定](./soulbinding.md) + `restrictions: level:20`。
...
- 入门基线，仅 `glow`、无 `unbreakable`（[属性与数值](./attributes-numbers.md)）。
...
- `tier: LEGENDARY` 高配无动作（[品质与显示](./quality-display.md)）。
...
- 三段 [处理器](./action-system/handlers.md) 串联：`qi:title` + `qi:sound` + `qi:bossbar`，`left_click` [触发器](./action-system/triggers.md) + `cooldown: 3s`。
...
- 唯一演示 `consume`（`item:redstone:1` 释放时消耗 1 颗红石作法力石）的装备，配 `cooldown: 2s` + `restrictions: level:15`，含 `qi:subtitle` [处理器](./action-system/handlers.md)（[触发器](./action-system/triggers.md)）。
...
- 入门基线，连 `options` 块都没有（[属性与数值](./attributes-numbers.md)）。
...
- `equip` / `unequip` 双 [触发器](./action-system/triggers.md) 演示披甲 / 卸甲提示（[处理器](./action-system/handlers.md)）。
...
- 双 [触发器](./action-system/triggers.md)：`left_click`（擦拭，带 `cooldown: 3s`）+ `equip`（披甲号角）。`max_health` 属性（[属性与数值](属性与数值.md）），`bind_on_acquire` 获得即 [灵魂绑定](./soulbinding.md)（此处不配 `restrictions`）。

---

## 下一步

- [示例-饰品消耗篇](./cookbook-accessories-consumables.md)——饰品、消耗品、料理等类型示例
- [示例-杂项篇](./cookbook-misc.md)——工具、方块、杂项类型示例
- [物品示例库总览](./item-cookbook.md)——返回示例库目录
