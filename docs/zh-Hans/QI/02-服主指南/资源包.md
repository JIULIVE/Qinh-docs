...
> 所属：[服主指南](./item-definition.md)　·　相关：[物品定义](./item-definition.md) · [碎片与模板](./fragments-templates.md)
...
> 也可以只用 `options.custom_model_data`（见 [物品定义 → options](./item-definition.md#3-options-选项段)），但若 `resource_pack.custom_model_data` 也设了，以后者为准。
...
GUI 里选模型时（[资源包编辑器](../03-editor-gui/other-editors.md)）从这里取候选。
...
支持的前缀（`ce` / `ia` / `nexo` / `mi` / `ni` / `mm` …）与完整语法、排错见 [物品定义 → 5.1 外部物品源引用](./item-definition.md#51-外部物品源引用craftengine--itemsadder--nexo-等)。
...
- [物品定义](./item-definition.md)：options 视觉字段
- [编辑器 GUI → 其他编辑器](../03-editor-gui/other-editors.md)：资源包 / 模型选择器
