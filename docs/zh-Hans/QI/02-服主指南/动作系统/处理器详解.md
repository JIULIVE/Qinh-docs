...
> 所属：[动作系统](./overview.md)　·　相关：[处理器大全](./handlers.md) · [触发器实战](./triggers-hands-on.md)

[处理器大全](./handlers.md) 是速查表，本页对每个内置处理器**逐个展开**：payload 各字段含义、多种写法变体、易错点。
...
- 需 `combat.enabled: true`。见 [属性与数值](../attributes-numbers.md)。
...
- 在套装 `abilities` 里 handler 必须用 **map 形式**（含冒号），见 [套装](../sets.md)。
- 见 [集成](../../04-developer/integration.md)。
...
[开发一个自定义处理器](../../04-developer/handler-development.md)：实现 `QinhActionHandler`，逻辑写在代码里，YAML 引用它即可。这是把 if/switch/状态判断引入动作的唯一正道（YAML 不能写逻辑）。

---

## 下一步

- [处理器大全](./handlers.md)：速查表
- [触发器实战](./triggers-hands-on.md)：触发 + 处理器组合
- [动作处理器开发](../../04-developer/handler-development.md)
