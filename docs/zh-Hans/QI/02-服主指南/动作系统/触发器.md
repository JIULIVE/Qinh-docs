...
> 所属：[动作系统](./overview.md)　·　相关：[处理器](./handlers.md) · [冷却/消耗/条件](./cooldown-cost-conditions.md)

触发器原子（trigger atom）决定「玩家做什么会触发动作」。在 `trigger.atom` 处填写。本页按类别列出全部内置原子（来自 `TriggerType` / `TriggerSpec`）。

> 📋 想要每类触发器一个**复制即用的最小 YAML**？看 [触发器实战示例](./triggers-hands-on.md)。
...
详见 [冷却/消耗/条件/连招](./cooldown-cost-conditions.md#4-序列连招与双击)。

---

## 下一步

- [处理器](./handlers.md)：触发后执行什么
- [冷却/消耗/条件](./cooldown-cost-conditions.md)：限制触发
