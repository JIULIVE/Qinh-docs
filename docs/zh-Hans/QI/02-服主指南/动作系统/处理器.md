...
> 所属：[动作系统](./overview.md)　·　相关：[触发器](./triggers.md) · [动作处理器开发](../../04-developer/handler-development.md)

处理器（Handler）是触发后真正执行的「效果」。在 `refs` 列表里用 `handler` + `payload` 引用。本页列出全部内置处理器及其 payload 格式。

> 🔎 想要每个处理器的**字段详解 + 多种写法变体 + 易错点**？看 [动作处理器详解](./handlers-in-depth.md)。
...
**qinhskills:cast** —— 把技能释放交给 [QinhSkills](../../04-developer/integration.md)：
```yaml
- handler: qinhskills:cast
  payload: '{"skill":"fireball","level":1}'
```

**combat:swing** —— 配合 [属性系统](../attributes-numbers.md) 的挥击：
```yaml
...
处理器的 payload 有两种模式（由其[载荷 schema](../../04-developer/handler-development.md#payload-schema) 决定）：

- **PLAIN**：单字段，payload 就是裸字符串（如 `qi:message` 的 `你好`）。
- **JSON**：多字段，payload 是 JSON 对象（如 `qinhskills:cast` 的 `{"skill":"...","level":1}`）。

GUI 编辑器会根据 schema 自动生成表单（[动作编辑器](../../03-editor-gui/action-editor.md)）。

---

## 开发自定义处理器

需要内置处理器之外的效果？实现 `QinhActionHandler` 接口并注册。这是把复杂逻辑（if/switch/状态）引入动作系统的**唯一正道**（YAML 里不能写逻辑）。详见 [动作处理器开发](../../04-developer/handler-development.md)。
...
- [冷却 / 消耗 / 条件 / 连招](./cooldown-cost-conditions.md)
- [动作处理器开发](../../04-developer/handler-development.md)：自定义处理器 + payload schema
