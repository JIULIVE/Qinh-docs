...
> 所属：[服主指南](./item-definition.md)　·　相关：[品质与显示](./quality-display.md) · [段](./sections.md) · [词缀](./affixes.md)
...
参数都可省略（不填则随机 Tier、默认池）。见 [命令](../05-reference/commands.md)。
...
容量参数来自 Tier 的 `generation.capacity`（见 [品质](./quality-display.md#3-tier-字段表)）：
...
对应命令 `/qi refresh`（及 `all` / `inventory` / `armor` / `equipment` / `hand`），见 [命令](../05-reference/commands.md)。刷新 = 对每件 QI 物品调 `assembly().rebuild()` + `variables().refresh()`，保留实例 / 种子、只更新显示。

---

## 下一步

- [品质与显示](./quality-display.md)：配 Tier 的 chance / capacity
- [段](./sections.md) / [词缀](./affixes.md)：配词缀池
