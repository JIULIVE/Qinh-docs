...
> 所属：[服主指南](./item-definition.md)　·　相关：[段](./sections.md) · [随机生成](./random-generation.md) · [品质与显示](./quality-display.md)

词缀是**可复用的修饰包**：给物品名加前缀 / 后缀、追加一行 Lore。配合[段（Section）](./sections.md)的池机制，可实现随机词缀。

---

## 1. 词缀是什么

一个词缀（`QinhAffix`）本质是一个携带「前缀 / 后缀 / Lore + 任意元数据」的命名容器。物品通过 `affixes:` 列表引用它。词缀通常和[段](./sections.md)、[随机生成](./random-generation.md)一起用。
...
- 取 `lore` 追加为一行 Lore（落在[结构化 Lore](./quality-display.md#5-lore-段顺序从上到下) 的「段 Lore」段）。
...
简单说：**词缀是「料」，段是「怎么抽料」**。要随机前后缀，用段的 `weight_join`/`quality_pool` 池；要固定挂一个词缀，直接 `affixes:` 引用。详见 [段](./sections.md)。
...
管理命令：`/qi affixes`、`/qi affixes summary`、`/qi affixes categories`、`/qi affixes formats`，见 [命令](../05-reference/commands.md)。

---

## 下一步

- [段](./sections.md)：加权随机池、品质池
- [随机生成](./random-generation.md)：按容量从池抽词缀
