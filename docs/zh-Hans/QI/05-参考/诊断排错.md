...
> 所属：[参考](./commands.md)　·　相关：[校验报错速查](./validation-errors.md) · [命令](./commands.md)
...
报错含义见 [校验报错速查](./validation-errors.md)。
...
详见 [属性与数值](../02-server-guide/attributes-numbers.md#6-没装-attributeplus-会怎样)。
...
详见 [集成实操 → Legendinlay](../04-developer/integration-howto.md#三legendinlay宝石孔后端-a)。

---

## 五、健康度指标解读

物品 `healthIssues()` 为空才算健康。不健康物品仍加载但列入问题。常见不健康原因见 [校验报错速查 §1](./validation-errors.md#1-物品健康检查qinhitemdefinitionhealthissues)。
...
刷新 = 对每件 QI 物品 `rebuild()` + `variables().refresh()`，保留实例 / 种子。见 [随机生成 → 刷新服务](../02-server-guide/random-generation.md#7-物品刷新服务)。

---

## 下一步

- [校验报错速查](./validation-errors.md)：每条报错 + 修法
- [命令](./commands.md)：诊断命令
- [FAQ 常见问题集](./faq.md)
