...
> 所属：[参考](./commands.md)　·　相关：[诊断排错](./diagnostics.md) · [命令](./commands.md)
...
刷新 = 每件 `rebuild()` + `variables().refresh()`，物品越多越重。见 [诊断排错 §6](./diagnostics.md#六刷新相关)。
...
见 [层与装配](../04-developer/layers-assembly.md) 与 [校验报错速查](./validation-errors.md)。
...
见 [导入导出实操](../04-developer/import-export-howto.md#五内容包qipack跨服分发)。
...
- [导入导出实操](../04-developer/import-export-howto.md)
