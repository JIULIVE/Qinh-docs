...
> 所属：[编辑器 GUI](./overview.md)　·　相关：[动作系统](../02-server-guide/action-system/overview.md) · [物品编辑器](./item-editor.md)
...
载荷 schema 是开发者为处理器声明的字段结构，详见 [动作处理器开发 → payload schema](../04-developer/handler-development.md#payload-schema)。
...
技能机制见 [集成 → QinhSkills](../04-developer/integration.md)。
...
GUI 里配的就是 [动作系统](../02-server-guide/action-system/overview.md) 那套 `actions.triggers`：
...
- [动作系统](../02-server-guide/action-system/overview.md)：底层概念
- [处理器](../02-server-guide/action-system/handlers.md)：处理器与 payload
- [动作处理器开发](../04-developer/handler-development.md)：自定义处理器 + schema
