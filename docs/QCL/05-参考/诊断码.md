...
> 导航：[文档首页](../README.md) · [目录](../README.md) · [诊断与排错](../02-服主指南/诊断与排错.md) · [FAQ](FAQ.md) · [术语表](术语表.md) · [模块系统](../04-开发者/模块系统.md)
...
> 一个桥可以「启用但不可用」（开关开着，但对应插件没装 → `NO_HOOK`），也可以「未启用」（被 `modules.*` 关掉）。排错时先分清是哪一层。详见 [模块系统](../04-开发者/模块系统.md)。
...
| `PARSE_FAILED` | 引用串解析失败 | 引用格式错误（空串、非法分隔等） | 核对引用格式，见 [物品源引用](../02-服主指南/物品源引用.md) |
...
> 🔎 `/qcl status` 末尾的「物品引用诊断」用 `vanilla:stone` 做**自检**——它返回 `OK` 表示物品源解析链路本身正常；某个具体引用失败要单独用 `ItemManagerAPI.diagnose(ref, player)` 排查。详见 [物品 API](../04-开发者/物品API.md)。
...
详见 [脚本入门](../02-服主指南/脚本入门.md)、[脚本 API](../04-开发者/脚本API.md)。
...
详见 [经济动作](../02-服主指南/经济动作.md)、[经济插件](../03-外部插件对接/经济插件.md)、[经济 API](../04-开发者/经济API.md)。
...
构造用伴生方法 `DiagnosticResult.ok(...)` / `DiagnosticResult.fail(...)`。详见 [模块系统 · 诊断模型](../04-开发者/模块系统.md)。
...
- [诊断与排错](../02-服主指南/诊断与排错.md) — `/qcl status` 逐行解读与排查决策树
- [FAQ](FAQ.md) — 常见问答
- [模块系统 · 诊断模型](../04-开发者/模块系统.md) — `DiagnosticResult` / `HealthReport` / `BridgeStatus`
- [术语表](术语表.md) — 名词解释
