> 上一页：[自定义GUI.md](自定义GUI.md) · 下一页：[经济动作.md](经济动作.md)
> 相关：[脚本入门.md](脚本入门.md) · [经济动作.md](经济动作.md) · [GUI编程API.md](../04-开发者/GUI编程API.md)

# 🔎 GUI 动作与条件速查

这是一页**纯速查表**：所有点击动作（`type`）、所有显隐条件（`view-requirement`），每个都附可直接照抄的 YAML 片段。GUI 基本结构请先看 [自定义GUI.md](自定义GUI.md)。
...
| `give_money` | `givemoney` | 见[经济动作.md](经济动作.md) | 给钱 |
| `take_money` | `takemoney`,`remove_money` | 见[经济动作.md](经济动作.md) | 扣钱 |
| `set_money` | `setmoney` | 见[经济动作.md](经济动作.md) | 设置余额 |
...
> 💰 `give_money` / `take_money` / `set_money` 三个经济动作的 value 写法较复杂（含 provider、currency、失败提示），单独整理在 **[经济动作.md](经济动作.md)**。
...
> 另有 `shift: true` 可叠加，表示「仅 Shift + 点击」时才触发（见 [自定义GUI.md](自定义GUI.md)）。
...
> ⚠️ 上面三条动作按顺序执行。若想做到「扣钱失败就不发物品」这种**原子事务**，请改用 `javascript` 动作把扣钱与发物品写在一个脚本里，见 [脚本入门.md](脚本入门.md)。
...
## 📖 继续阅读
- 🖼️ [自定义GUI.md](自定义GUI.md) — GUI 完整结构与字段
- 💰 [经济动作.md](经济动作.md) — 经济三动作的 value 语法
- 📜 [脚本入门.md](脚本入门.md) — `javascript` 条件与动作
- 🧰 [GUI编程API.md](../04-开发者/GUI编程API.md) — 开发者扩展动作与数据源
