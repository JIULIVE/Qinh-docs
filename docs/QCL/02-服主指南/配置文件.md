> 上一页：[安装](../01-入门/安装.md)　·　下一页：[命令与权限](命令与权限.md)
...
> 🔗 在 GUI 等地方写经济动作时，可以用更精确的格式覆盖默认值，例如 `excellenteconomy:gold:50`，详见 [命令与权限](命令与权限.md) 与开发文档 [经济API](../04-开发者/经济API.md)。
...
- **依赖被禁模块的功能会失效**。例如禁用 `item` 后，[统一物品源引用](物品源引用.md)、GUI 物品引用、子插件的物品 API 都会受影响。
...
## 📖 继续阅读
- ➡️ 下一页：[命令与权限](命令与权限.md) — `/qcl reload` 到底重载了哪些东西
- 🧩 [物品源引用](物品源引用.md) — 依赖 `item` 模块的核心功能
- 🩺 [诊断与排错](诊断与排错.md) — 用 `/qcl status` 检查各模块状态
- 📚 [术语表](../05-参考/术语表.md)　·　[FAQ](../05-参考/FAQ.md)
